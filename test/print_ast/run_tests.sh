#!/bin/bash
#
# Test runner for --print-ast output
# Compiles each test .c file with --print-ast and compares output
...
for src in "$SCRIPT_DIR"/test_*.c; do
    ...
    asm_file="${name}.asm"
    if [ -f "$asm_file" ]; then
        echo "FAIL  $name  (.asm file should not be generated with --print-ast)"
        rm -f "$asm_file"
        fail=$((fail + 1))
        continue
    fi
