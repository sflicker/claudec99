# Full Grammar Parse Tree — Stage 105
...
(gcc-built) compiles itself to C1, and C1 compiles itself to C2, with all 1594
tests passing at each step (see `docs/self-compilation-report.md`).

**Stages 91-01 through 104 carry no expression-grammar or AST-shape changes**
(except inside the constant-expression evaluator subsystem). Stages 91-01
(struct-by-value), 93/94 (build workflow / self-host cycle), and the entire
95-xx series are codegen/ABI/tooling work. Stages 96-98 add language features
(multi-file compilation, designated initializers, compound literals). Stages
100-104 extend the constant-expression evaluator.

**Stage 105 adds five preprocessor features and one parser feature** — no new
AST node types, and no changes to the main expression-parser function chain.
The only parser change is the `__func__` predefined identifier, which is handled
in `parse_primary` and synthesizes an `AST_STRING_LITERAL` node (the same node
type used by ordinary string literals).

---

**Stage 105 — C99 Preprocessor Completion.**

Five C99 features added to `src/preprocessor.c` and `src/parser.c`:

1. **`#pragma`** — unknown pragmas are silently ignored; `#pragma once` records
   the source file path in `MacroTable.once_paths` and causes `preprocess_file`
   to return an empty string on subsequent includes of that path.

2. **`_Pragma("string")`** — the C99 operator form: the string is destringized
   (backslash-escape sequences resolved) and applied as a pragma. Only `"once"`
   is recognized (same effect as `#pragma once`); all other strings are silently
   ignored. `_Pragma(...)` expands to nothing (zero preprocessing tokens).

3. **`#line digit-sequence ["filename"]`** — overrides `current_line` (set to
   `new_line - 1` so the newline ending the directive line brings it to
   `new_line`) and, when a quoted filename is present, `current_file_override`
   (a new heap-allocated local, freed before return). Subsequent `__LINE__` and
   `__FILE__` expansions respect these overrides.

4. **Null directive** — a `#` followed by optional whitespace and then a newline
   is silently consumed (C99 §6.10.7).

5. **`__func__`** — recognized in `parse_primary` (before `__builtin_va_*`)
   as a predefined identifier. Outside a function body (`current_func_name ==
   NULL`) it is a `PARSER_ERROR`. Inside a function body it synthesizes an
   `AST_STRING_LITERAL` node whose `node->value` is the function name interned
   via `lexer_store_bytes`, `byte_length` is the name length, and `full_type` is
   `type_array(type_char(), len + 1)` — identical in every respect to a string
   literal and handled identically by codegen and all downstream passes.

   The `Parser` struct gains a `const char *current_func_name` field
   (initialized to `NULL` in `parser_init`). The function-definition arm in
   `parse_external_declaration` saves/restores it around the `parse_block` call,
   setting it to `d.name` for the duration of the function body. Nested function
   definitions (not legal in C99, but guarded in the parser) restore correctly
   because the save/restore is stack-local.
...
└─► parse_external_declaration
       ...
       Stage 105: while parsing a definition body, current_func_name is set
         to d.name (saved/restored) so __func__ can be resolved to the
         enclosing function's name as a string literal.
                                                    ...
                                                    └─► parse_unary
                                                        ...
                                                        └─► parse_postfix
                                                            ...
                                                            └─► parse_primary
                                                                   ...
                                                                   __func__ (stage 105)
                                                                     C99 §6.4.2.2 predefined identifier;
                                                                     matched before __builtin_va_* and
                                                                     before enum/identifier dispatch.
                                                                     Outside a function body → PARSER_ERROR
                                                                       "used outside of a function body"
                                                                     Inside a function body → AST_STRING_LITERAL
                                                                       node->value = lexer_store_bytes(fname, len)
                                                                       byte_length = len
                                                                       decl_type = TYPE_ARRAY
                                                                       full_type = type_array(type_char(), len+1)
                                                                     Identical in type and storage to a
                                                                     regular string literal; decays to
                                                                     `const char *` in expression contexts.
...
- **Complete constant-expression evaluator (stage 104):** both `eval_const_expr`
  ...
  bug was also fixed. No parser, AST, or grammar changes.
- **C99 preprocessor completion (stage 105):** `#pragma` (unknown pragmas ignored,
  `#pragma once` guards against double inclusion), `_Pragma("str")` operator,
  `#line` directive (overrides `__LINE__` and `__FILE__` expansions), null
  directive (`#` alone), and `__func__` predefined identifier. No new AST node
  types; `__func__` synthesizes an `AST_STRING_LITERAL` identical to a regular
  string literal. `MacroTable` gains three `once_paths` fields for
  `#pragma once` tracking; `Parser` gains `current_func_name`.
...
- **Self-hosting (stages 92–105):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1594-test suite, confirming bootstrap stability. See `docs/self-compilation-report.md`.
