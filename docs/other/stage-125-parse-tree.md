...
# Full Grammar Parse Tree — Stage 125
...
**Stages 100–112 carry no expression-grammar, parser function, or AST node
changes.** All work in this range is in the preprocessor, lexer (new token
kinds for float/double literals), type system, and codegen:

- **Stages 100–108** are tooling, preprocessor, and stub-header improvements.
  Stage 108 adds `#elifdef` / `#elifndef` (preprocessor-only).
- **Stage 109** adds `float` and `double` scalar types.
  `parse_type_specifier` gains two new keyword branches:
  `"float"` → `TYPE_FLOAT` and `"double"` → `TYPE_DOUBLE`.
  The lexer gains `TOKEN_FLOAT_LITERAL` (with `f`/`F` suffix) and
  `TOKEN_DOUBLE_LITERAL` (no suffix); both are recognized in `parse_primary`
  and produce `AST_FLOAT_LITERAL` / `AST_DOUBLE_LITERAL` nodes.
  `sizeof(float)` = 4, `sizeof(double)` = 8.
- **Stages 110–111** add FP arithmetic, UAC, conversions, and comparison
  operators — all codegen-only; no new AST node types or grammar rules.
- **Stage 112** adds the full SysV AMD64 FP calling convention: FP arguments
  in `xmm0`–`xmm7`, FP return values in `xmm0`, variadic register-save area
  for XMM regs, `va_arg(ap, double)` — all codegen-only; no parser, grammar,
  or AST changes.

**Stages 113–119 carry no expression-grammar or AST-shape changes beyond the two items below.**
Stage 113 is a test-suite reorganization (no compiler source changes). Stage 116 is a
codegen-only bug fix for global struct arrays (BSS sizing and char[N] string-literal init).
Stage 117 is a codegen-only bug fix for FP struct member rvalue reads. Stage 118 is a
codegen-only addition of pointer relational operators (`<`, `<=`, `>`, `>=`) via unsigned
setcc variants — no new tokens, AST nodes, or grammar rules (the parser already accepted
these operators). Stage 119 is a codegen-only fix adding `codegen_find_global` fallbacks
in `expr_result_type()`, `sizeof_type_of_expr()`, and `emit_arrow_addr()` so that FP
compound assignment on file-scope struct fields and global pointer-to-struct arrow access
work correctly — no parser, AST, or grammar changes in either stage.
The two parser-facing changes in this range are:

- **Stage 114**: `AST_STRING_LITERAL` is now accepted as a subscript base in
  `parse_postfix` and `emit_array_index_addr` in codegen, enabling `"abc"[2]`.
  Parser: the postfix suffix loop's `TOKEN_LBRACKET` branch now applies to
  `AST_STRING_LITERAL` nodes (was gated to structured lvalue types only). Codegen:
  `emit_array_index_addr` gains a case for `AST_STRING_LITERAL` that loads the
  string pool pointer (adding the index for direct subscript access).
- **Stage 115**: Array dimension bounds in `parse_declarator` and `parse_type_name`
  now call `eval_const_expr(parser, "array dimension")` instead of requiring a bare
  `TOKEN_INT_LITERAL`. Enables `int a[N*2]`, `int a[sizeof(int)*8]`, `int a[MACRO]`.
  Four sites in `src/parser.c` were updated; the evaluator itself is unchanged
  (available since stage 99).

**Stages 120–122 carry no expression-grammar, parser function, or AST node changes.**
All three stages are codegen-only:

- **Stage 120** adds FP `++`/`--` support in `codegen_inc_dec_general`: an FP
  early-return path using `movsd`/`movss`, `addsd`/`addss`, `subsd`/`subss`, and
  two new `.rodata` constants (`Lfp_one_f64`, `Lfp_one_f32`). Affects struct member
  access via dot (`.`) and arrow (`->`), FP array elements, and FP pointer dereferences.
- **Stage 121** fixes the `switch` dispatch loop to use 64-bit compare instructions
  (`mov rax, [rsp]` / `cmp rax, <val>`) when the discriminant type is `TYPE_LONG`,
  `TYPE_LONG_LONG`, or `TYPE_UNSIGNED_LONG_LONG`; `int`/`char`/`short` discriminants
  continue to use the 32-bit path (they are integer-promoted before evaluation).
- **Stage 122** fixes ABI compliance by preserving the callee-saved `rbx` register
  across function calls. Every function prologue now emits `mov [rbp - 8], rbx`
  (after `sub rsp`) and every epilogue emits `mov rbx, [rbp - 8]` (before
  `mov rsp, rbp`). `cg->stack_offset` starts at 8 (reserving `[rbp - 8]`);
  variadic functions add 8 bytes of alignment padding before the 176-byte XMM
  register-save area so that `movaps` XMM slots remain 16-byte aligned. All 21
  `test/print_asm/` expected files were regenerated to reflect the new instructions
  and shifted local-variable offsets.

**Stages 123–125 add three parser-facing changes and are otherwise codegen-only:**

- **Stage 123**: `parse_external_declaration` now accepts `AST_VAR_REF` (function
  designators, e.g., `void *fp = some_func;`) and `AST_ADDR_OF` (address-of
  expressions, e.g., `int *p = &global_var;`) as valid initializers for
  pointer-typed file-scope variables. Previously only integer literals, string
  literals, and `NULL` (zero) were accepted; function designators and address-of
  expressions triggered "non-constant initializer" errors. The remaining stage 123
  work is pure codegen: FP stack argument fixes (CC99-001/002), indirect FP call
  layout (CC99-003), address-constant `.data` emission (CC99-004), and FP
  short-circuit evaluation (CC99-005).
- **Stage 124**: Two parser-facing changes:
  1. **Octal integer literals**: `src/lexer.c` detects a leading `0` followed by
     an octal digit (0–7) after the decimal digit path. Digits 8 and 9 in an octal
     literal trigger a compile error ("invalid octal digit"). The value is parsed
     via `strtoul` with base 8, then rewritten to a decimal string via `snprintf`
     (NASM does not understand C99 octal notation such as `0755`). No new AST node
     type — the result is still `AST_INT_LITERAL` with decimal text in `node->value`.
  2. **File-scope compound literals (pointer types only)**: the early-rejection guard
     in `parse_external_declaration` that emitted "compound literals at file scope
     are not yet supported" for any `AST_COMPOUND_LITERAL` is replaced with a
     type-specific guard: only non-pointer compound literals at file scope are
     rejected. Pointer-typed globals may now have an `AST_COMPOUND_LITERAL`
     initializer, and `AST_COMPOUND_LITERAL` is added to the valid list for the
     pointer-global initializer path. Codegen (`codegen_emit_data`) gains two new
     sub-cases for this form. `__func__` is codegen-only (stage 106/107 already
     handled parsing `__func__` as a VAR_REF in `parse_primary`).
- **Stage 125**: `parse_external_declaration` extends the `TYPE_FLOAT`/`TYPE_DOUBLE`
  global initializer branch to accept `AST_INT_LITERAL` in addition to
  `AST_FLOAT_LITERAL`/`AST_DOUBLE_LITERAL`. Negated integer literals arriving as
  `AST_UNARY_OP("-", AST_INT_LITERAL("7"))` are folded: the parser converts the
  integer string to a `long` via `strtol`, negates it, formats it back via
  `snprintf`, and interns the result with `lexer_store_bytes`, storing an
  `AST_INT_LITERAL` with the negated decimal value. Variadic float→double
  promotion is codegen-only.
...
└─► parse_external_declaration
       ...
       Stage 99: file-scope initializer uses parse_assignment_expression (not
         ...
         (stage 124: pointer-type compound literals at file scope are now accepted;
         only non-pointer compound literals retain this error)
       Stage 123: global pointer initializer now also accepts AST_VAR_REF
         (function designators) and AST_ADDR_OF (address-of expressions) in
         addition to integer/string literals; codegen emits `dq label` /
         `dq label + offset` for these forms
       Stage 124: octal integer literals recognized by lexer (0[0-7]+);
         AST_COMPOUND_LITERAL accepted for pointer-type file-scope globals
       Stage 125: TYPE_FLOAT/TYPE_DOUBLE global initializer branch accepts
         AST_INT_LITERAL; negated integer literals folded to AST_INT_LITERAL
         with the negated decimal value
    ...
    │          float/double param (stage 112): full_type set to float/double;
    │            codegen binds xmmN → local stack slot in prologue
...
│   "long" | "signed" | "unsigned" | "float" | "double" |   (stage 109)
│   "enum" | "struct" | "union" | <typedef-name> )
...
│   │      float/double variable: decl_type=TYPE_FLOAT/TYPE_DOUBLE (stage 109)
│   │
...
│      Stage 112: when the function returns float/double, codegen places the
│        result in xmm0.
│      Stage 122: all return paths emit `mov rbx, [rbp - 8]` before `mov rsp, rbp`
│        to restore the callee-saved rbx register per the SysV AMD64 ABI.
...
│      Stage 121: after codegen_expression for the discriminant, captures
│        disc_kind = node->children[0]->result_type; emits `mov rax, [rsp]` /
│        `cmp rax, <val>` for TYPE_LONG/TYPE_LONG_LONG/TYPE_UNSIGNED_LONG_LONG;
│        keeps `mov eax, [rsp]` / `cmp eax, <val>` for int/char/short.
                                                        ...
                                                        └─► parse_postfix
                                                            ...
                                                            └─► parse_primary
                                                                   integer literal   → AST_INT_LITERAL
                                                                     ...
                                                                     0[0-7]+ octal (stage 124) — value
                                                                       rewritten to decimal string for NASM;
                                                                       digits 8/9 in octal → compile error
                                                                     suffix-typed; is_unsigned set for U/u
                                                                   float literal     → AST_FLOAT_LITERAL
                                                                     decimal with f/F suffix (stage 109)
                                                                   double literal    → AST_DOUBLE_LITERAL
                                                                     decimal without f/F suffix (stage 109)
                                                                   ...
                                                                   __builtin_va_* intrinsics (stage 75-03),
                                                                     ...
                                                                     __builtin_va_arg(ap, <type_name>)
                                                                       ...
                                                                       stage 112: va_arg(ap, float) rejected
                                                                         per C99 §6.5.2.2p6; va_arg(ap, double)
                                                                         emits fp_offset read from reg-save area
                                                                   enum constant
                                                                   ...
                                                                   direct function call
                                                                     ...
                                                                     struct/union value args marshalled per
                                                                       ...
                                                                       (stage 91-01)
                                                                     float/double args passed in xmm0–xmm7
                                                                       (stage 112); before variadic calls,
                                                                       al = count of xmm regs used;
                                                                       float variadic args promoted to double
                                                                       via cvtss2sd (stage 125)
                                                                   ...
                                                                   identifier (variable reference) → AST_VAR_REF
                                                                     __func__ → AST_VAR_REF resolved by codegen
                                                                       to LocalVar injected at function entry
                                                                       (stage 107 parse; stage 124 codegen)
...
parse_type_name                            (cast, sizeof(type), __builtin_va_arg type)
   ...
   abstract_declarator: { "*" [ "const" | "volatile" ] } [ "[" [const_expr] "]" { "[" const_expr "]" } ]
     ...
     Stage 115: bracket dimensions call eval_const_expr (was TOKEN_INT_LITERAL only)
   accepts: void, _Bool, scalar keywords, signed/unsigned types, typedef names,
            "struct"/"union" tags, "enum" tags, float, double (stage 109)
...
parse_declarator                           (declarations and parameters)
   Leading stars with optional per-pointer const/volatile (stage 66/82-04):
     { "*" [ "const" | "volatile" ] }
   Plain declarator:
     { "*" } <identifier> { "[" [ <const_expr> ] "]" }        (multidim — stage 86; const-expr bounds stage 115)
       dims collected into array_dims[]/dim_count (MAX_ARRAY_DIMS=8);
       each bound evaluated by eval_const_expr (stage 115: was TOKEN_INT_LITERAL only)
...
parse_parameter_declaration                (inside parameter lists)
   ...
   float/double parameter (stage 112): decl_type=TYPE_FLOAT/TYPE_DOUBLE; codegen
     moves xmmN register to local stack slot in prologue
...
- **Compound literals (stage 99):** `parse_cast` now detects `(type-name) {`
  ...
  `stack_size`. File-scope pointer-type compound literals are now accepted (stage
  124); non-pointer file-scope compound literals still trigger "compound literals
  at file scope are not yet supported".
- **Array-dimension constant expressions (stage 115):** four sites in
  `parse_declarator` and `parse_type_name` that previously required
  `TOKEN_INT_LITERAL` for array bounds now call `eval_const_expr(parser,
  "array dimension")`, accepting any C99 integer constant expression:
  arithmetic, bitwise, shift, relational, equality, logical AND/OR, ternary,
  `sizeof`, parentheses, enum constants, and macro identifiers. The evaluator
  itself is unchanged (available since stage 99). Runtime variables as
  dimensions → "is not an integer constant expression". Negative or
  zero-valued constant dimensions → "array size must be greater than zero".
- **String literal subscript (stage 114):** the postfix suffix loop's
  `TOKEN_LBRACKET` branch now applies to `AST_STRING_LITERAL` nodes, enabling
  `"abc"[2]` expressions. Codegen's `emit_array_index_addr` handles the
  case by loading the string pool pointer then indexing.
- **`float` and `double` types (stage 109):** `parse_type_specifier` gains
  `"float"` → `TYPE_FLOAT` and `"double"` → `TYPE_DOUBLE` branches.
  `parse_primary` recognizes `TOKEN_FLOAT_LITERAL` (with `f`/`F` suffix) and
  `TOKEN_DOUBLE_LITERAL` (no suffix), producing `AST_FLOAT_LITERAL` and
  `AST_DOUBLE_LITERAL`. All codegen effects (SSE2 loads/stores, FP arithmetic
  in stages 110–112) are orthogonal to the parser.
- **FP calling convention (stage 112):** no parser or AST changes. Codegen's
  `compute_call_layout` now receives a fifth parameter (`const TypeKind *actual_types`)
  derived from `expr_result_type()` so that FP arguments in variadic extras are
  classified correctly (rather than using `decl_type` which is TYPE_VOID on
  expression nodes). Variadic prologues save `xmm0`–`xmm7` to a 176-byte
  register-save area (48 GP + 128 XMM, 16-byte aligned at `[rbp-128]`).
  `va_arg(ap, float)` is rejected per C99 §6.5.2.2p6 (float promotes to double).
  `test/valid/run_tests.sh` gains `.libs` companion-file support for extra link
  flags (e.g., `-lm` for `sqrt`, `pow`).
- **Switch on 64-bit discriminants (stage 121):** the `switch` dispatch loop in
  `codegen_statement` captures `disc_kind = node->children[0]->result_type` after
  evaluating the discriminant. For `TYPE_LONG`, `TYPE_LONG_LONG`, and
  ...
  before evaluation). Case label values remain limited to 32-bit signed integers.
- **Callee-saved rbx preservation (stage 122):** every function frame now
  reserves `[rbp - 8]` for saving and restoring `rbx`. `cg->stack_offset` starts
  at 8; variadic functions add 8 bytes of alignment padding before the 176-byte
  XMM save area (so `movaps` slots remain 16-byte aligned). The prologue emits
  `mov [rbp - 8], rbx` after `sub rsp`; all four epilogue paths emit
  `mov rbx, [rbp - 8]` before `mov rsp, rbp`. This satisfies the SysV AMD64
  ABI requirement that `rbx` be callee-saved — critical for callbacks to external
  code (e.g., `qsort` comparators) that relies on `rbx` being preserved.
- **Address-constant global initializers (stage 123):** `parse_external_declaration`
  now validates `AST_VAR_REF` (function designators) and `AST_ADDR_OF` (address-of
  expressions) as legal initializers for pointer-typed globals. Codegen stores
  these as label-based initializers and emits `dq label` / `dq label + offset`
  in `codegen_emit_data`. `LocalStaticVar` gains `is_label_init`/`init_label`
  fields so block-scope static pointers can also use this path.
- **Octal integer literals (stage 124):** the lexer detects `0[0-7]+` sequences;
  digits 8 or 9 in an octal context are a compile error. Values are parsed with
  `strtoul(base=8)` then rewritten to decimal via `snprintf` for NASM output.
  No new AST node type — the result is `AST_INT_LITERAL` with the decimal value.
- **`__func__` predefined identifier (stage 124):** `codegen_function` now
  injects a `LocalStaticVar` (label `Lstatic_<name>___func__<N>`, `TYPE_ARRAY`,
  contains the function name as a null-terminated byte string) and a `LocalVar`
  named `__func__` pointing to it at the entry of every function. The VAR_REF
  lookup in `parse_primary` (added in stage 106/107) then resolves `__func__`
  to this local. `compound_literal_count` in `CodeGen` is reused as the label
  suffix counter.
- **File-scope pointer compound literals (stage 124):** `AST_COMPOUND_LITERAL`
  initializers for pointer-typed globals are now accepted. Codegen in
  `codegen_emit_data` handles Case A (array compound literal → anonymous
  `Lcompound_N` label + `dq`) and Case B (address-of compound literal →
  anonymous label + `dq`). Non-pointer file-scope compound literals remain
  unsupported.
- **FP globals from integer initializers (stage 125):** `parse_external_declaration`
  accepts `AST_INT_LITERAL` in the `TYPE_FLOAT`/`TYPE_DOUBLE` initializer branch.
  Negated integer literals (parsed as `AST_UNARY_OP("-", AST_INT_LITERAL("7"))`)
  are folded: the parser converts to `long` via `strtol`, negates, formats via
  `snprintf`, and interns with `lexer_store_bytes`. Codegen converts the integer
  string to `%.17g` (double) / `%.9g` (float), appends `.0` if no decimal point
  or exponent is present, and stores via `init_label` so NASM emits the correct
  IEEE 754 encoding (e.g., `dq 5.0` not `dq 5`).
- **Variadic float→double promotion (stage 125):** `compute_call_layout` assigns
  `s->nbytes = 8` (not 4) for `float` variadic extra arguments (when
  `callee->is_variadic && actual_types[i] == TYPE_FLOAT`). Both the Phase 1
  (stack-overflow) and Phase 2 (XMM-register) emission paths emit
  `cvtss2sd xmm0, xmm0` before storing a float variadic arg, promoting it to
  double per C99 §6.5.2.2p7.
...
- **Self-hosting (stages 92–125):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1919-test suite (1233 valid, 262 invalid, 88 integration, 50 print-AST,
  100 print-tokens, 21 print-asm, 165 unit), confirming bootstrap stability.
  The original bootstrap (stages 91-01/92) surfaced and fixed thirteen real
  defects; stage 94 added the repeatable `build.sh --mode=self-host` C0→C1→C2
  cycle; subsequent stages each re-verified self-hosting. See
...
- **Still parser-rejected (known gaps):** functions returning function pointers;
  ...
  `va_arg` for `float` is rejected per C99 (float args are promoted to double
  in variadic calls).
