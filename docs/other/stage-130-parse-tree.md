...
# Full Grammar Parse Tree — Stage 130
...
- **Compound literals (stage 99):** `parse_cast` now detects `(type-name) {`
  ...
  remain rejected.
- **Self-hosting (stages 92–130):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1,531-test suite, confirming bootstrap stability. The original bootstrap
  ...
  required a one-time bootstrap fix (multi-declarator array split). All stages
  from 95 through 130 verified self-hosting with no additional source changes.
