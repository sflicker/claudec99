...
# Full Grammar Parse Tree — Stage 117
...
**Stages 113–117 carry no expression-grammar or AST-shape changes beyond the two items below.**
...
Stage 117 is a codegen-only bug fix for FP struct member rvalue reads (`double`/`float`
fields read via dot, arrow, and subscript-then-dot) — all fixes are in `codegen_expression()`,
`expr_result_type()`, and `sizeof_type_of_expr()`; no parser, AST, or grammar changes.
...
- **Self-hosting (stages 92–117):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1863-test suite (1181 valid, 258 invalid, 88 integration, 50 print-AST,
  ...
  source uses no struct FP fields in arithmetic, so stage 117's codegen fixes
  have no effect on the bootstrap output. See `docs/self-compilation-report.md`.
