# Full Grammar Parse Tree — Stage 138
...
**Stage 134 extends `parse_struct_specifier` to handle bit-field members
(CC99-006) and flexible array members (CC99-007).** (Stage 134)

1. **Bit-field member parsing**: after parsing a declarator in the struct
   member loop, `parse_struct_specifier` checks for a `:` token. If found,
   it consumes the width expression (an integer constant via
   `eval_const_expr`). Three cases are distinguished:
   - **Named bit-field** (`unsigned int x : N`): the declarator has an
     identifier; the field is packed into a storage unit whose size is
     determined by the field's base type alignment. `bf_bits_used` tracks how
     many bits are already occupied in the current storage unit.
   - **Anonymous bit-field** (`: N` with no declarator): adds padding bits
     without contributing a named field.
   - **Zero-width bit-field** (`: 0`): forces the next named field to begin
     a new storage unit (GCC-compatible alignment/packing behavior).
   State variables `bf_unit_offset`, `bf_unit_size`, and `bf_bits_used` are
   maintained across the member loop to implement GCC-compatible layout.
   `StructField` in `include/type.h` gains four new fields: `is_bitfield`,
   `bit_width`, `bit_offset`, and `is_flexible_array`.

2. **Flexible array member parsing** (Stage 134): a member declarator whose
   array dimension list has `has_size=0` (i.e., `type name[]`) is recognized
   as a flexible array member when it is the last named member. Validation:
   - Flexible array member must not be the first (or only) named member.
   - No further named members may follow it in the struct.
   `sizeof` excludes flexible array storage (the field contributes zero bytes
   to the struct size). Indexed access compiles via existing array-decay-to-
   pointer codegen: `name` decays to a pointer to element zero, enabling
   `p->data[i]` accesses.

3. **Codegen — bit-field rvalue read** (Stage 134): `emit_member_addr` detects
   `StructField.is_bitfield`. Instead of emitting an address, the rvalue path
   loads the storage unit (sized load at the field's `byte_offset`), shifts
   right by `bit_offset`, and masks with `(1 << bit_width) - 1` to extract
   the field value into `rax`.

4. **Codegen — bit-field lvalue write** (Stage 134): the store path performs a
   read-modify-write: load the storage unit, clear the target bits with
   `(mask << bit_offset)`, OR in the new value shifted left by `bit_offset`,
   and store the updated unit back. All accesses are sized to the storage-unit
   width determined at layout time.

---

**Stage 135 adds pointer-to-array declarators and type-compatibility adjustments
in parameter lists (CC99-008, CC99-009).** (Stage 135)

- **`parse_declarator`** now parses `(*name)[N]` and `(*name)[]` patterns
  instead of rejecting them. `ParsedDeclarator` gains three new fields:
  `is_ptr_to_array` (set to 1), `ptr_to_array_length` (the dimension `N`, or 0
  for unsized), and `ptr_to_array_has_size`. This enables parameter types like
  `int (*row)[4]` and local declarations like `int (*p)[4] = &a`.
- **`parse_parameter_declaration`** now applies array-to-pointer (C99 §6.7.5.3p7)
  and function-to-pointer-to-function (C99 §6.7.5.3p8) adjustments for named
  declarators before compatibility checks are performed. This allows compatible
  redeclarations where one uses an array form (`int values[3]`) and another uses
  a pointer form (`int *values`).
- Fixed a pre-existing bug: `(void)` in function-pointer parameter lists was
  counted as one void parameter rather than zero, causing spurious arity mismatches.
- **No new AST node types. Codegen unchanged** — indexed access on a
  pointer-to-array (`(*row)[i]`) already worked via the existing deref + subscript
  codegen path.

**Stage 136 fixes a bug in `sizeof_type_of_expr` for pointer-arithmetic
expressions.** (Stage 136)

- **Codegen only**: two fixes in `sizeof_type_of_expr` in `src/codegen.c`:
  (1) added a pointer/array guard in the `AST_BINARY_OP` case for `+`/`-`
  operators, returning `TYPE_POINTER` (size 8 on LP64) when either operand is
  a pointer or array type; (2) added an `AST_STRING_LITERAL` case returning
  `TYPE_POINTER` so string literals in binary expressions are treated as
  pointers.
- **No parser, grammar, AST node, or token changes.**

**Stage 137 ships support for functions returning function pointers
(`(*get_adder())(int)` declarator form), resolving CC99-010.** (Stage 137)

- **`parse_declarator`** now parses `(*name())(params)` fully instead of
  rejecting it. `ParsedDeclarator` gains four new fields: `is_func_returning_fp`
  (set to 1), `own_param_types` (the outer parameter list types),
  `own_param_count` (the outer parameter count), and `own_is_no_prototype`.
  A guard on `inner_stars == 0` remains to reject the invalid direct-function
  return form `int (f())(int)` ("direct function return type not supported").
- **`parse_external_declaration`** gains an `is_func_returning_fp` branch: it
  constructs the nested pointer-to-function return type, creates
  `AST_FUNCTION_DECL` with `decl_type = TYPE_POINTER`, registers the function,
  and optionally parses its body. Fixed a pre-existing bug: typedef'd pointer
  return types were not being recognized via the `func->full_type` assignment
  condition.
- **No new AST node types. Codegen unchanged** — function pointers and indirect
  calls already worked correctly; the new form just generates an
  `AST_FUNCTION_DECL` whose return type is `TYPE_POINTER`.

**Stage 138 adds `auto` and `register` storage-class specifiers (CC99-011,
CC99-012).** (Stage 138)

- **New tokens**: `TOKEN_AUTO` and `TOKEN_REGISTER` added to the lexer with
  keyword recognition and display names.
- **`parse_declaration_specifiers`** (file scope): `TOKEN_AUTO` and
  `TOKEN_REGISTER` are recognized but rejected at file scope with an error.
- **`parse_statement`** (block scope): `TOKEN_AUTO` causes the inner declaration
  to be tagged `SC_AUTO = 8` (treated identically to plain automatic storage);
  `TOKEN_REGISTER` tags the declaration `SC_REGISTER = 16`.
- **`parse_parameter_declaration`**: `register` is accepted as a leading
  qualifier (the keyword is consumed and the storage class recorded; no semantic
  effect on the parameter type).
- **AST/Codegen**: `SC_AUTO` and `SC_REGISTER` added to the `StorageClass` enum
  in `include/ast.h`; `ast_pretty_printer` displays both. `LocalVar` gains an
  `is_register` field; `AST_ADDR_OF` rejects address-of on register variables
  with a compile error ("cannot take address of register variable").
...
└─► parse_external_declaration
       ...
       Stage 137: is_func_returning_fp branch — when the declarator has
         is_func_returning_fp=1, constructs the nested pointer-to-function return
         type, creates AST_FUNCTION_DECL with decl_type=TYPE_POINTER, registers
         the function in the function table, and optionally parses its body.
         Fixed pre-existing bug: typedef'd pointer return types were not
         recognized via the func->full_type assignment condition.
    ...
    │      Stage 138: TOKEN_AUTO and TOKEN_REGISTER recognized but rejected at
    │        file scope ("auto/register not allowed at file scope")
    ...
    │          Stage 135: array-to-pointer and function-to-pointer adjustments
    │            applied for named declarators before compatibility checks, so
    │            int values[3], int values[], and int *values are all compatible
    ...
    │          Stage 138: "register" accepted as leading qualifier; SC_REGISTER
    │            recorded on the parameter (no semantic or codegen effect beyond
    │            tracking)
    │       └─► parse_type_specifier
    │
...
│
├─► TOKEN_AUTO (block scope)  →  Stage 138: explicit auto (default storage)
│      Consumes "auto", recursively parses the inner declaration, and marks
│      each resulting AST_DECLARATION with storage_class = SC_AUTO (= 8).
│      Generates identical code to a plain automatic declaration.
│   └─► parse_statement  (the inner declaration)
│
├─► TOKEN_REGISTER (block scope)  →  Stage 138: register hint
│      Consumes "register", recursively parses the inner declaration, and
│      marks each resulting AST_DECLARATION with storage_class = SC_REGISTER
│      (= 16). Codegen sets LocalVar.is_register = 1. Address-of (&) on a
│      register variable → compile error "cannot take address of register variable".
│      Generates otherwise identical code to a plain automatic declaration.
│   └─► parse_statement  (the inner declaration)
│
                                                ...
                                                └─► parse_cast
                                                    ...
                                                    └─► parse_unary
                                                           ...
                                                           & accepts AST_VAR_REF, AST_ARRAY_INDEX,
                                                             ...
                                                             Stage 138: & on a register variable →
                                                               compile error "cannot take address of
                                                               register variable"
                                                           sizeof forms:
                                                             "sizeof" <unary_expr>
                                                             "sizeof" "(" <type_name> ")"
                                                             ...
                                                             Stage 136: sizeof_type_of_expr fixed for
                                                               pointer-arithmetic expressions (ptr + int,
                                                               ptr - int now return TYPE_POINTER size;
                                                               string literals in binary exprs return
                                                               TYPE_POINTER)
...
eval_const_expr                            (integer constant expression evaluator)
   ...
   Stage 134: also used for bit-field width expressions in parse_struct_specifier.
...
parse_struct_specifier
   "struct" [<tag>] "{" <struct_declaration_list> "}"
     struct_declaration:
       ...
       Stage 134: after each declarator, checks for ":" (bit-field width marker):
         ":" <width_expr>  where width_expr is an integer constant (eval_const_expr)
         named bit-field (declarator has identifier, width > 0):
           packs into storage unit of the field's base-type alignment;
           bf_unit_offset / bf_unit_size / bf_bits_used track packing state;
           StructField gains is_bitfield=1, bit_width=N, bit_offset=B
         anonymous bit-field (no declarator, width > 0):
           advances bf_bits_used by width; no StructField added
         zero-width bit-field (width == 0): forces new storage unit boundary;
           next named field begins at a freshly aligned offset
         GCC-compatible layout: if a new bit-field does not fit in the current
           storage unit, the unit is closed and a new one started at the next
           aligned offset
       Stage 134: flexible array member detection:
         declarator with no array size (has_size=0, is_array=1, dim_count==1):
           validated as a flexible array member (must not be first/only named member)
           StructField gains is_flexible_array=1; contributes 0 bytes to struct size
           access via existing array-decay-to-pointer codegen path
           further named members → "flexible array member must be last"
...
parse_declarator                           (declarations and parameters)
   ...
   Parenthesized declarator forms:
     ...
     (*name)[N]  pointer-to-array declarator (stage 135):
       is_ptr_to_array=1, ptr_to_array_length=N (or 0 for unsized),
       ptr_to_array_has_size=1 (or 0 for unsized)
       (was: error "pointer-to-array declarators not yet supported")
     (*name())(param-list)  functions returning function pointers (stage 137):
       is_func_returning_fp=1; own_param_types/own_param_count/own_is_no_prototype
       populated from the outer "(param-list)" parameter list;
       inner_stars must be > 0 (inner_stars == 0 → error "direct function return
       type not supported — use a typedef or function pointer")
       (was: error "functions returning function pointers not yet supported")
   returns ParsedDeclarator:
     ...
     is_ptr_to_array, ptr_to_array_length, ptr_to_array_has_size  (stage 135)
     is_func_returning_fp, own_param_types, own_param_count,
       own_is_no_prototype  (stage 137)
...
parse_declaration_specifiers               (file-scope)
   ...
   Stage 106: "restrict" consumed and discarded in pointer-qualifier positions
   Stage 107: "inline" consumed and discarded
   Stage 138: TOKEN_AUTO and TOKEN_REGISTER recognized; both rejected at file scope
     with "error: auto/register storage class not allowed at file scope"
...
parse_parameter_declaration                (inside parameter lists)
   [ "const" | "volatile" | "register" ] <type_specifier> [ <declarator> | { "*" } ]
   ...
   Stage 135: for named declarators, array-to-pointer (§6.7.5.3p7) and
     function-to-pointer-to-function (§6.7.5.3p8) adjustments are applied
     before compatibility checks, so int values[3], int values[], and int *values
     all form compatible function parameter types. Fixed pre-existing bug:
     (void) in function-pointer parameter lists now correctly counted as zero
   ...
   Stage 138: leading "register" keyword accepted as a storage-class qualifier;
     SC_REGISTER recorded, no semantic or codegen effect on the parameter.
...
- **Constant-expression evaluators (stages 77, 99–104, 115, 128):** two
  ...
  - `eval_const_expr` (token-stream, integer): nine-level recursive descent
    ...
    dimension bounds (stage 115), file-scope integer initializers (stage 100),
    bit-field width expressions (stage 134), and block-scope static scalar
    initializers (stage 103); extended through stage 104 to cover relational,
...
- **Default argument promotions for no-prototype calls (stage 133):**
  ...
  definition.
- **Bit-field members (stage 134):** `parse_struct_specifier` detects `:` after
  a declarator (or `:` with no preceding declarator for anonymous bit-fields).
  GCC-compatible packing: named bit-fields share storage units as long as they
  fit; a field that does not fit opens a new aligned unit; zero-width forces a
  new unit boundary. Four new `StructField` fields (`is_bitfield`, `bit_width`,
  `bit_offset`, `is_flexible_array`) carry the layout. Codegen reads via
  shift+mask and writes via RMW (read-clear-or-store).
- **Flexible array members (stage 134):** a trailing `name[]` declarator in a
  struct is detected by `has_size=0` in the `ParsedDeclarator`. Validation
  ensures it is not the first/only named member and that no members follow it.
  `sizeof` excludes the flexible array. Existing array-decay-to-pointer codegen
  handles indexed access (`p->data[i]`).
- **Pointer-to-array declarators (stage 135):** `parse_declarator` detects the
  `(*name)[N]` form via the parenthesized-declarator path. `ParsedDeclarator`
  gains `is_ptr_to_array`, `ptr_to_array_length`, and `ptr_to_array_has_size`
  fields. No new AST node; the resulting `TYPE_POINTER → TYPE_ARRAY` type chain
  is assembled in the semantic layer using existing type constructors. Indexed
  access `(*p)[i]` works via existing deref + subscript codegen.
- **Parameter type adjustments (stage 135):** `parse_parameter_declaration`
  applies C99 §6.7.5.3p7 (array-to-pointer) and §6.7.5.3p8 (function-to-pointer)
  adjustments for named parameters before compatibility checking, enabling
  compatible redeclarations with differing array/pointer spellings.
- **Functions returning function pointers (stage 137):** `parse_declarator`
  detects `(*name())(params)` by looking for inner_stars > 0 followed by `()`
  and a trailing outer parameter list. `ParsedDeclarator` gains `is_func_returning_fp`
  and `own_param_*` fields. `parse_external_declaration` builds the nested
  `TYPE_POINTER → TYPE_FUNCTION` return type and creates a normal
  `AST_FUNCTION_DECL` whose `decl_type = TYPE_POINTER`. No codegen changes —
  the function is called and returned from identically to any other
  pointer-returning function.
- **`sizeof` of pointer-arithmetic expressions (stage 136):** `sizeof_type_of_expr`
  in codegen now returns `TYPE_POINTER` (8 bytes on LP64) when either operand
  of a `+`/`-` binary expression is a pointer or array, and similarly for
  string literals in binary expressions. Before this fix, `sizeof(p + 1)` and
  `sizeof("str" + 1)` returned the element size rather than the pointer size.
- **`auto` and `register` storage classes (stage 138):** `TOKEN_AUTO` and
  `TOKEN_REGISTER` are recognized by the lexer and handled in both
  `parse_declaration_specifiers` (file scope — rejected) and `parse_statement`
  (block scope — accepted, generating `SC_AUTO` and `SC_REGISTER`
  `AST_DECLARATION` nodes). `register` enforces no-address-of at compile time
  via `LocalVar.is_register`; both generate identical code to plain automatic
  declarations otherwise. `parse_parameter_declaration` also accepts `register`
  as a leading qualifier with no semantic effect.
...
- **Compound literals (stage 99):** `parse_cast` now detects `(type-name) {`
  ...
  remain rejected.
- **Self-hosting (stages 92–138):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1,970-test suite, confirming bootstrap stability. The original bootstrap
  ...
  `test/include/stdlib.h`). All stages from 95 through 138 verified
  self-hosting with no additional source changes beyond those two bootstrap
  fixes. See `docs/self-compilation-report.md`.
- **Still parser-rejected (known gaps):** old-style (K&R) function definitions;
  chained designators (`.a.b`, `.arr[0]`); designated union init for non-first
  members; non-pointer compound literals at file scope; `__attribute__` specifiers.
