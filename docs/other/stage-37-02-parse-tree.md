...
# Full Grammar Parse Tree — Stage 37-02
...
└─► parse_external_declaration
    ...
    │      Collects one optional storage class + one type specifier.
    │      storage class: "extern" | "static" | "typedef"
    │      Returns DeclSpecResult {sc, base_kind, base_type}
    ...
    │          unnamed parameter when next token is "," or ")"
    │          AST_PARAM  (name in node->value, possibly empty for unnamed)
...
│
├─► "char" | "short" | "int" | "long" | "enum" | "struct" | <typedef-name>
...
│   │        brace-list: AST_INITIALIZER_LIST child (stage 32)
│   │        omitted size requires string-literal initializer
│   │
                                        ...
                                        └─► parse_additive
                                            ...
                                            └─► parse_term
                                                ...
                                                └─► parse_cast
                                                       ...
                                                       type_name accepts: int types,
                                                         typedef names, struct types
                                                            ...
                                                            └─► parse_primary
                                                                   ...
                                                                   direct function call
                                                                     ...
                                                                     arguments: parse_assignment_expression
                                                                                { "," parse_assignment_expression }
                                                                     arity is validated against the registered signature
...
parse_type_name                            (used in cast and sizeof(type) contexts)
   <type_specifier> { "*" }
   accepts: scalar keywords, typedef names, "struct" tags
└─► parse_type_specifier
...
parse_declaration_specifiers               (file-scope only)
   Collects one storage class + one type specifier in any order.
   storage class: "extern" | "static" | "typedef"
...
parse_parameter_declaration                (inside parameter lists)
   <type_specifier> [ <declarator> ]
   declarator is optional: absent when next token is "," or ")"
   Named parameter: AST_PARAM (name in node->value)
   Unnamed parameter: AST_PARAM (empty node->value)
...
- `parse_declaration_specifiers` at file scope handles `extern`, `static`, and
  `typedef` storage classes in combination with any type specifier; duplicate
  storage class or duplicate type specifier within a single declaration is an error.
