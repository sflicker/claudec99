...
# Full Grammar Parse Tree — Stage 116
...
**Stages 113–116 carry no expression-grammar or AST-shape changes beyond the two items below.**
...
- **Self-hosting (stages 92–116):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1857-test suite, confirming bootstrap stability. The original bootstrap
  ...
  `build.sh --mode=self-host` C0→C1→C2 cycle, and subsequent stages each
  re-verified self-hosting. See `docs/self-compilation-report.md`.
