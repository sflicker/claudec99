...
# Full Grammar Parse Tree — Stage 122
...
- **Self-hosting (stages 92–122):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1894-test suite (1210 valid, 260 invalid, 88 integration, 50 print-AST,
...
- **Still parser-rejected (known gaps):** functions returning function pointers;
  ...
  for non-first members; compound literals at file scope. `va_arg` for `float`
  is rejected per C99 (float args are promoted to double in variadic calls).
