...
# Full Grammar Parse Tree — Stage 119
...
- **Self-hosting (stages 92–119):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1879-test suite (1195 valid, 260 invalid, 88 integration, 50 print-AST,
  ...
  cycle; subsequent stages each re-verified self-hosting. The compiler's own
  source uses no file-scope struct FP fields in arithmetic, so stages 117–119's
  codegen fixes have no effect on the bootstrap output. See
