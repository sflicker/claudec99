...
# Full Grammar Parse Tree — Stage 97
...
- **Self-hosting (stages 92–97):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1501-test suite, confirming bootstrap stability. The original bootstrap
  ...
  checks in `vec_reserve`/`vec_push`/`strbuf_append_char`). Stages 96 and 97
...
- **Still parser-rejected (known gaps):** functions returning function pointers;
  ...
  for non-first members; inline struct/union literals (`(T){ … }`). Block-scope
  static arrays remain unsupported in codegen.
