...
# Full Grammar Parse Tree — Stage 98
...
└─► parse_external_declaration
       ...
       Stage 98: file-scope initializer uses parse_assignment_expression (not
...
│   │        Stage 98: initializer may now be an AST_COMPOUND_LITERAL (e.g.
...
└─► parse_initializer_element  (stage 97: each list element)
    ...
    ├─► eval_case_const_expr  (for array index designator — forward-declared before
    │     parse_initializer; same evaluator as case-label constant expressions)
    └─► parse_initializer  (the designator's value)
└─► parse_assignment_expression  (scalar / non-brace elements)

build_compound_literal                     (stage 98: builds AST_COMPOUND_LITERAL)
   ...
   all paths: node->byte_length = 0 (assigned by pre-scan before codegen)
              ...
              child[0] = initializer
└─► infer_compound_literal_array_length    (stage 98)
...
parse_type_name                            (cast, sizeof(type), __builtin_va_arg type)
   ...
   abstract_declarator: { "*" [ "const" | "volatile" ] } [ "[" [N] "]" { "[" N "]" } ]
     ...
     Stage 98: "[" "]" (omitted first dimension, length=0) accepted for compound
...
- **Constant-expression `case` labels (stage 77):** a dedicated constant
  ...
  `case ENUM_CONST:` and `case 'A' + 1:` are now accepted. The same evaluator is
  reused by `parse_initializer_element` for array designator indices (stage 97).
...
- **Self-hosting (stages 92–98):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1521-test suite, confirming bootstrap stability. The original bootstrap
  ...
  stages each re-verified self-hosting. Stages 96, 97, and 98 all passed
...
- **Still parser-rejected (known gaps):** functions returning function pointers;
  ...
  arrays remain unsupported in codegen.
