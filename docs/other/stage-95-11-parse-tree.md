...
**Stages 91-01 through 95-11 carry no expression-grammar or AST-shape changes.**
...
old 255-byte string-literal cap entirely. None of this changes which programs
parse; the parser functions below are unchanged in dispatch and only differ in
how their symbol tables and value strings are stored.
...
- **Self-hosting (stages 92–95-11):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1479-test suite, confirming bootstrap stability. The original bootstrap
  ...
  checks in `vec_reserve`/`vec_push`/`strbuf_append_char`). See
