...
# Full Grammar Parse Tree — Stage 112
...
- **Self-hosting (stages 92–112):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1650-test suite, confirming bootstrap stability. The original bootstrap
...
- **Still parser-rejected (known gaps):** functions returning function pointers;
  ...
  arrays remain unsupported in codegen. `va_arg` for `float` is rejected per
  C99 (float args are promoted to double in variadic calls).
