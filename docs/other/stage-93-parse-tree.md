...
# Full Grammar Parse Tree — Stage 93
...
(gcc-built) compiles itself to C1, and C1 compiles itself to C2, with all 1306
tests passing at each step (see `docs/self-compilation-report.md`). Stages 91-01
and 93 carry no expression-grammar changes (struct-by-value is a codegen/ABI
feature; stage 93 is a build-workflow change).
...
│        (MAX_SWITCH_LABELS raised 64 → 256 in stage 92 for token_type_name().)
                                                        ...
                                                        └─► parse_postfix
                                                            ...
                                                            └─► parse_primary
                                                                   ...
                                                                   string literal    → AST_STRING_LITERAL (char[N+1])
                                                                     ...
                                                                     (MAX_STRING_LITERALS raised 256 → 2048 — stage 92)
...
- **Self-hosting (stage 92):** the compiler now compiles itself. C0 (gcc-built)
  produces C1, and C1 produces C2; all three pass the full 1306-test suite,
  confirming bootstrap stability. The bootstrap surfaced and fixed thirteen real
  defects across stages 91-01/92 (struct-by-value ABI, the silent AST-truncation
  bug, six struct-by-value/`sizeof` codegen bugs, duplicate-`extern` emission,
  literal/switch-label capacity limits, missing `global` directives, and `strtol`
  stubs). See `docs/self-compilation-report.md`.
