...
# Full Grammar Parse Tree — Stage 109
...
- **`float`/`double` types (stage 109):** `TOKEN_FLOAT` and `TOKEN_DOUBLE` are
  ...
  const-expr sizeof) include `TOKEN_FLOAT`/`TOKEN_DOUBLE`. FP arithmetic,
  comparisons, and function parameters remain out of scope (stages 110–112).
- **Self-hosting (stages 92–109):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1627-test suite, confirming bootstrap stability. The original bootstrap
  ...
  100–109 all passed self-hosting with no new issues. See
...
- **Still parser-rejected (known gaps):** functions returning function pointers;
  ...
  arithmetic, comparisons, and function parameters remain out of scope.
