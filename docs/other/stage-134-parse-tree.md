...
# Full Grammar Parse Tree — Stage 134
...
- **Compound literals (stage 99):** `parse_cast` now detects `(type-name) {`
  ...
  remain rejected.
- **Self-hosting (stages 92–134):** the compiler compiles itself. C0
  (gcc-built) produces C1, and C1 produces C2; all three pass the full
  1,946-test suite, confirming bootstrap stability. The original bootstrap
  ...
  `test/include/stdlib.h`). All stages from 95 through 134 verified
