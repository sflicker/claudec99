...
in `src/peephole.c` (pure string-parsing logic, no new headers, C89-style
variable declarations) and the expanded `g_builtin_patterns[5]` array compile
cleanly under C0. All 2070 portable tests + 187 system-include tests + 2
optional-library tests (test_sdl2_init, test_zlib_compress) passed at C0, C1,
and C2 with no source changes needed during bootstrap.
...
| Step | Binary | Version | Tests |
|------|--------|---------|-------|
| C0 | `build/ccompiler-c0` | `00.03.01670000.01230` | 2070 portable + 187 sysinclude + 2 optional |
| C1 | `build/ccompiler-c1` | `00.03.01670000.01231` | 2070 portable + 187 sysinclude + 2 optional |
| C2 | `build/ccompiler-c2` | `00.03.01670000.01232` | 2070 portable + 187 sysinclude + 2 optional |
...
functions in `src/peephole.c` (pure string-parsing logic, no new headers,
C89-style variable declarations) and the expanded `g_builtin_patterns[3]`
array compile cleanly under C0. All 2068 portable tests + 185 system-include
tests + 2 optional-library tests (test_sdl2_init, test_zlib_compress) passed
...
| Step | Binary | Version | Tests |
|------|--------|---------|-------|
| C0 | `build/ccompiler-c0` | `00.03.01650000.01219` | 2068 portable + 185 sysinclude + 2 optional |
| C1 | `build/ccompiler-c1` | `00.03.01650000.01220` | 2068 portable + 185 sysinclude + 2 optional |
| C2 | `build/ccompiler-c2` | `00.03.01650000.01221` | 2068 portable + 185 sysinclude + 2 optional |
...
inserted in `optimize_expr`. Task 1 (`!!x`) uses `ast_new`, `ast_add_child`,
`util_strdup`, and `strcmp` — all available in C0. Task 2 (binary boolean) uses
`strtol`, `strcmp`, `ast_new`, `ast_free`, and `util_strdup` — likewise
available in C0. No VLAs, no `//` comments, all declarations at the top of
their respective `if` blocks. No new AST node types, no parser changes, no
codegen changes. All 126 tests passed at C0, C1, and C2 with no source
changes needed.
...
| Step | Binary | Version | Tests |
|------|--------|---------|-------|
| C0 | `build/ccompiler-c0` | `00.03.01470000.01100` | 126/126 |
| C1 | `build/ccompiler-c1` | `00.03.01470000.01101` | 126/126 |
| C2 | `build/ccompiler-c2` | `00.03.01470000.01102` | 126/126 |
...
replaced with a unified block handling `-`, `+`, `!`, and `~`. All new code
uses `strtol`, `snprintf`, `strcmp`, `ast_new`, `ast_free`, and `util_strdup`
— all available in C0. No new AST node types, no parser changes, no codegen
changes. All 1992 tests passed at C0, C1, and C2 with no source changes needed.
...
| Step | Binary | Version | Tests |
|------|--------|---------|-------|
| C0 | `build/ccompiler-c0` | `00.03.01440000.01083` | 1992/1992 |
| C1 | `build/ccompiler-c1` | `00.03.01440000.01084` | 1992/1992 |
| C2 | `build/ccompiler-c2` | `00.03.01440000.01085` | 1992/1992 |
...
`static const char` initializer inside `src/preprocessor.c` (the built-in
preamble string). ClaudeC99 does not yet support adjacent string literal
concatenation, so the multi-line form was collapsed into a single long string
literal. No other source changes were needed. All 1979 tests passed at C0, C1,
and C2.

## Result (stage 139 supplemental)
...
| Step | Binary | Version | Tests |
|------|--------|---------|-------|
| C0 | `build/ccompiler-c0` | `00.03.13900000.01046` | 1979/1979 |
| C1 | `build/ccompiler-c1` | `00.03.13900000.01047` | 1979/1979 |
| C2 | `build/ccompiler-c2` | `00.03.13900000.01048` | 1979/1979 |
