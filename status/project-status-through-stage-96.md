# ClaudeC99 Project Status — Through Stage 96

_As of 2026-06-08 (commit `9b9630c`)_
...
**Stages completed: 203** (stage-01 through stage-96, including substages).
...
All 1,483 tests pass with no regressions — under the gcc-built compiler and
...
| **96**     | **Multi-file compilation; per-file teardown (current)**       |

## Recently Shipped (Stage 96)

**Stage 96 — multi-file compilation and per-file lifecycle.**

The driver (`src/compiler.c`) now accepts one or more positional source
files on the command line. For each file it calls `reset_error_state()` to
zero the per-file error counters, then `compile_one_file()` — a new
`static` helper that owns the complete per-file Lexer/Parser/CodeGen/AST
cycle and calls `parser_free`, `codegen_free`, `lexer_free`, `ast_free`,
and `free(preprocessed)` on all exit paths. The overall exit status
accumulates the per-file statuses.

Three new lifecycle functions were added:

- **`parser_free(Parser *)`** (`src/parser.c`): `vec_free`s the seven
  Vecs the parser owns (`funcs`, `globals`, `typedefs`, `enum_consts`,
  `enum_tags`, `struct_tags`, `union_tags`). Element strings remain in
  lexer-owned storage.
- **`codegen_free(CodeGen *)`** (`src/codegen.c`): frees all
  codegen-generated strings (via the new `Vec owned_strings` field and
  `codegen_intern()` helper that routes all `util_strdup` calls through a
  single tracked list) then `vec_free`s all eight Vecs in `CodeGen`.
- **`reset_error_state()`** (`src/util.c`): zeroes `g_error_count`,
  `g_error_src_file/line/col`, and `g_error_jmp_valid` between files.

Error isolation works with `--max-errors=0`: a compile error in one file
does not terminate the process, so the remaining files are still compiled.
(With the default `--max-errors=1`, the first error calls `exit(1)`
directly before `longjmp` can fire — multi-file error isolation is
therefore meaningful only with `--max-errors=0`.)

Two integration tests were added under `test/integration/`:
`test_multi_file_success` (two files with same-named `static` helpers
compile independently, producing separate `.asm` files; a `main.c` links
them and verifies output) and `test_multi_file_error` (error in `bad.c`
is isolated; `good.asm` is still produced). The integration runner was
updated to support `run_test.sh`-based custom tests.

No language features or grammar rules changed.
...
in `docs/other/stage-96-parse-tree.md`, and the feature checklist in
