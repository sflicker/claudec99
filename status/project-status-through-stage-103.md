# ClaudeC99 Project Status — Through Stage 103

_As of 2026-06-11 (commit `20e3176`)_
...
**Stages completed: 210** (stage-01 through stage-103, including substages).
...
| Total LOC (src + include)     | 14,988         |
| Valid runtime tests           | 894            |
| Invalid (compile-error) tests | 253            |
...
All 1,569 tests pass with no regressions — under the gcc-built compiler and
...
- File-scope (global) variable declarations (`.bss` / `.data`,
  ...
  constant expressions for initializers** (stage 100).
- Storage-class specifiers `extern` and `static` at file scope, **plus
  ...
  stage 101: arrays/structs/unions; stage 103: full constant-expression
  initializers for scalars) that persist across calls and are emitted to
  `.bss` (uninitialized) or `.data` (initialized) with RIP-relative
  `[rel Lstatic_func_N]` addressing. **Designated initializers work in
  static arrays** (stage 102); **multidimensional static arrays supported**
  (stage 102).
...
### Integer constant expressions (stages 77, 99–103)
...
- **File-scope scalar initializers**: `int x = 1 + 2 * 3;` (stage 100).
- **Block-scope static scalar initializers**: `static int x = 1 + 2 * 3;` (stage 103).
...
## Stage-by-Stage Timeline (76–103)
...
| **103**    | **Block-scope static scalar constant-expression initializers (current)** |

## Recently Shipped (Stages 100–103)
...
in stage 99; stages 100–103 added 30 valid and 5 invalid tests combined. All
1569 tests pass. The self-host C0→C1→C2 cycle passed at all four stages with
no new issues.
...
in `docs/other/stage-103-parse-tree.md`, and the feature checklist in
