# ClaudeC99 Project Status — Through Stage 39

_As of 2026-05-13 (commit `1fe161f`)_
...
**Stages completed: 97** (stage-01 through stage-39).
...
| Total LOC (src + include)     | 7,526          |
| Valid runtime tests           | 498            |
| Invalid (compile-error) tests | 167            |
...
All 798 tests pass with no regressions.
...
- Brace-enclosed initializer lists for local arrays and structs (partial
  initialization zero-fills remaining elements/members).
...
- Declaration specifier parsing accepts a list of specifiers, detecting
...
| **39**    | **Minimal `const` qualifier support (current)**               |

## Recently Shipped (Stages 29–39)

**Stage 29** added C-style enum support. Enumerator constants are
registered globally and folded to `AST_INT_LITERAL` at parse time;
anonymous and named enum declarations with auto-incrementing or explicit
literal values are supported.

**Stage 30** added struct definitions with natural-alignment field layout
and `sizeof` support for struct types.

**Stage 31** added `.` and `->` member access operators in both rvalue and
lvalue contexts, with new `AST_MEMBER_ACCESS` and `AST_ARROW_ACCESS` nodes
and `TYPE_STRUCT`-aware codegen.

**Stage 32** added brace-enclosed initializer lists for local arrays and
structs, with partial initialization zero-filling remaining
elements/members.

**Stage 33** added whole-struct copy via assignment and copy-initialization
from struct variables of the same type; byte-by-byte copy codegen with
strict type-pointer equality check.

**Stage 34** extended struct pointer parameters to support field access,
mutation via `->`, and `(*p).field` syntax; `emit_member_addr` extended
to handle `AST_DEREF` base expressions.

**Stage 35** added nested struct member access (chained dot, e.g.
`r.origin.x`) and arrays-of-structs element access (e.g. `points[0].x`);
two new `emit_member_addr` cases cover `AST_MEMBER_ACCESS` and
`AST_ARRAY_INDEX` bases.

**Stage 36** added `typedef` aliasing for complete struct types in both
separate-definition and combined typedef-struct definition forms.

**Stage 37** added incomplete struct forward declarations, enabling
self-referential structs (linked-list nodes) and opaque pointer fields;
the parser mutates the placeholder in-place when the body arrives so
typedef aliases remain valid. Stage 37-02 fixed chained arrow access
(`n.next->value`) and added incomplete-struct validation in the codegen.

**Stage 38** added `void` as a complete type: void functions, void
parameters, bare `return;`, `int f(void)` zero-parameter form, and
`void *` generic pointer with implicit conversion to/from any non-function
pointer type. Dereferencing, arithmetic, and subscripting void pointers
are rejected.

**Stage 39** added minimal `const` qualifier support: `const` is parsed
and stored on base-type scalars; direct assignments to const-qualified
lvalues are rejected at compile time. Full pointer-level const enforcement
is deferred to a later stage.

## Out of Scope (Not Yet Implemented)

These features remain explicitly outside the implemented language:
...
- Global array initialization (other than zero-fill via `.bss`).
- Calls with more than 6 arguments (calling-convention limit).
- Pointer subtraction (yielding `ptrdiff_t`).
- Multi-dimensional arrays.
- Unions.
- Pointer-level `const` enforcement (writes through `const *`, pointer
  conversion const-correctness).
- `typedef enum`.
- Enum constant expressions beyond integer/character literals.
- Pointer-to-function-pointer and functions returning function pointers.
- Calling through arbitrary function-pointer expressions (only direct
  variable and `(*fp)` forms are supported).
...
`docs/other/stage-21-03-parse-tree.md`.
