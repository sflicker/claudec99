# ClaudeC99 Project Status — Through Stage 102

_As of 2026-06-11 (commit `304fb57`)_
...
**Stage 102** completes **static aggregate coverage**, closing three gaps
deferred from stage 101 — all within `src/codegen.c`:
- **Designated initializers** in block-scope static array initializers
  (`static int arr[4] = {[2] = 99, [0] = 1}`) are now supported.
- **Static arrays of structs/unions** (`static struct Point pts[3] = {{1,2},{3,4}}`
  and uninitialized `static struct Pair pts[3]`) now emit correctly.
- **Multidimensional static arrays** (`static int grid[3][4]`, initialized
  `static int m[2][3] = {{1,2,3},{4,5,6}}`) now emit correct `.bss` and
  `.data` NASM. The same `.bss` bug is fixed for file-scope global
  multidimensional arrays.

**Stages completed: 209** (stage-01 through stage-102, including substages).
...
| Total LOC (src + include)     | 14,957         |
| Valid runtime tests           | 887            |
| Invalid (compile-error) tests | 251            |
...
All 1,560 tests pass with no regressions — under the gcc-built compiler and
...
- Storage-class specifiers `extern` and `static` at file scope, **plus
  ...
  in stage 101; **designated initializers, struct/union element types,
  and multidimensional (2D) integer arrays** added in stage 102.
...
### Block-scope static aggregates (stages 101–102)

Block-scope `static` local variables can now hold array, struct, and
union types. Stage 101 added the basic infrastructure; stage 102
completes coverage:
...
/* Stage 102: designated-init static array */
int get_flag(int i) {
    static int flags[4] = {[2] = 1, [0] = 1};
    return flags[i];
}

/* Stage 102: static array of structs */
int sum_pts(void) {
    static struct Point pts[3] = {{1,2},{3,4},{5,6}};
    return pts[0].x + pts[1].x + pts[2].x;
}

/* Stage 102: 2D static array */
...
Multidimensional static arrays (2D) now emit correct `.bss` (`resb
total_bytes`) and `.data` (row-by-row scalar emission). The same `.bss`
bug is fixed for file-scope global multidimensional arrays.

3D and deeper static arrays are not yet supported.
...
- **`[index] = value`** array index designators at both local and global
  scope (including block-scope static arrays — stage 102); the index must
  be a non-negative constant integer expression.
...
- Block-scope static aggregates with RIP-relative addressing, including
  designated-init arrays, struct/union element arrays, and 2D arrays.
...
## Stage-by-Stage Timeline (76–102)
...
| **102**    | **Complete static aggregate coverage (current)**              |

## Recently Shipped (Stage 102)

**Stage 102 — complete static aggregate coverage.**

Stage 102 closes three codegen gaps deferred from stage 101, all within
`src/codegen.c` (no parser, AST, or grammar changes needed):

**Task 1 — Designated initializers in static local array initializers.**
Guard A (which called `compile_error` on any `AST_DESIGNATED_INIT` child
in `codegen_emit_local_statics`'s array brace-list loop) is replaced with
the same index-designator handling used by the global array path. Now
`static int arr[4] = {[2] = 99, [0] = 1}` works at block scope.

**Task 2 — Static arrays of structs and unions.**
The slot-emit loop is extended with branches for struct elements (via
`emit_global_struct`), union elements (inline first-member init +
zero-fill), and null-slot zero-fill by byte count. Now `static struct
Point pts[3] = {{1,2},{3,4},{5,6}}` and uninitialized `static struct Pair
pts[3]` emit correctly.

**Tasks 3a/3b/3c — Multidimensional static arrays.**
- Task 3a: local static `.bss` for `static int grid[3][4]` now emits
  `resb 48` (total bytes) instead of the wrong `resd 3` (outer dim only).
- Task 3b: same two-case fix applied to `codegen_emit_bss` for file-scope
  global multidimensional arrays.
- Task 3c: initialized 2D static arrays (`static int m[2][3] = {{1,2,3},{4,5,6}}`)
  emit each row's scalars directly via `data_init_directive(scalar_type->kind)`.

**Test counts:** 7 new valid tests and 1 new invalid test. All 1560 tests pass.

The self-host C0→C1→C2 cycle passed with no new issues. The compiler's own
source uses no block-scope static aggregates of these new types.
...
- **3D and deeper static arrays** (`static int cube[2][3][4]` → error
  "deeper than 2D"); **static arrays of arrays of structs** (combines
  2D and struct-element emission — not yet supported).
