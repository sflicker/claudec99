# ClaudeC99 Project Status — Through Stage 122

_As of 2026-06-14 (commit `7a986d2`)_
...
**Stages completed: 243** (stage-01 through stage-122, including substages).
...
| Total LOC (src + include)     | 16,797         |
| Valid runtime tests           | 1,210          |
...
All 1,894 tests pass with no regressions — under the gcc-built compiler (C0)
...
### Floating-point types and calling convention (stages 109–122)
...
## Stage-by-Stage Timeline (76–122)
...
| **122**    | **Callee-saved `rbx` preserved per SysV AMD64 ABI (current)** |

## Recently Shipped (Stages 120–122)

**Stage 120 — FP `++`/`--` on Struct Members and FP Lvalues.** Extends
`codegen_inc_dec_general()` with an FP early-return path. When the target
lvalue is `TYPE_FLOAT` or `TYPE_DOUBLE`, the function emits a `movsd`/`movss`
load into `xmm0`, an `addsd`/`addss` or `subsd`/`subss` with a new `.rodata`
constant (`Lfp_one_f64` for `double`, `Lfp_one_f32` for `float`), and a
matching store. All target forms covered: dot (`.`), arrow (`->`), array
subscript, and pointer dereference. No tokenizer, parser, or AST changes.
7 new valid tests. All 1,886 tests pass.

**Stage 121 — 64-bit Switch Discriminants.** Fixes the `switch` dispatch loop
in `codegen_statement` for `long`, `long long`, and `unsigned long long`
discriminants. After evaluating the discriminant expression, `disc_kind` is
captured from the node's `result_type`. For 64-bit kinds the loop emits
`mov rax, [rsp]` / `cmp rax, <val>` (64-bit compare); `int`/`char`/`short`
continue to use the existing `mov eax, [rsp]` / `cmp eax, <val>` 32-bit path
(those types are integer-promoted to `int` before evaluation). Case label
values remain representable as 32-bit signed integers. No tokenizer, parser,
or AST changes. 6 new valid tests. All 1,892 tests pass.

**Stage 122 — Callee-Saved `rbx` Preservation.** Fixes an ABI violation:
the compiler uses `rbx` as a scratch register for address arithmetic (array
indexing, struct member access, lvalue `++`/`--`) via balanced push/pop pairs
within expression subtrees, but never preserved it across function call
boundaries — breaking callbacks called by external code such as glibc
`qsort`. The fix reserves `[rbp - 8]` in every function frame and adds
`mov [rbp - 8], rbx` to the prologue (after `sub rsp`) and
`mov rbx, [rbp - 8]` to all four epilogue code paths (bare `return;`,
struct-by-value return, normal `return <expr>;`, and implicit void fall-off).
`cg->stack_offset` starts at 8 instead of 0, shifting all local-variable
offsets by 8. Variadic functions get an extra 8-byte alignment pad before
the 176-byte XMM register-save area so `movaps` XMM slots remain 16-byte
aligned. `stack_size` computation adds +8 for the rbx slot; variadic
`stack_size` adds a second +8 for the alignment pad. All 21
`test/print_asm/` expected files regenerated. 2 new qsort callback tests.
No tokenizer, parser, or AST changes. All 1,894 tests pass.
...
├── codegen.c            Single-pass walker → NASM Intel-syntax asm; SysV ABI; SSE2 FP; rbx ABI
...
in `docs/other/stage-122-parse-tree.md`, and the feature checklist in
