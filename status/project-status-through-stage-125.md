# ClaudeC99 Project Status — Through Stage 125

_As of 2026-06-14 (commit `45f926d`)_
...
**Stage 117** fixes three codegen bugs causing **silent data corruption when
reading `double`/`float` struct fields as rvalues**: (1) `codegen_expression()`
rvalue blocks for `AST_MEMBER_ACCESS` and `AST_ARROW_ACCESS` emitted
`mov eax, [rax]` (4-byte integer) instead of `movsd xmm0, [rax]` /
`movss xmm0, [rax]` — fixed by adding FP early-return blocks; (2)
`expr_result_type()` called `promote_kind(TYPE_DOUBLE)` returning `TYPE_INT`,
causing binary operators on struct FP fields to take the integer code path —
fixed with `else if (type_is_fp(f->kind)) { t = f->kind; }` branches in the
`AST_MEMBER_ACCESS` and `AST_ARROW_ACCESS` cases; (3) `expr_result_type()` and
`sizeof_type_of_expr()` had no handler for `AST_ARRAY_INDEX` bases (`arr[i].x`)
and fell through to `TYPE_INT` — fixed by adding an `AST_ARRAY_INDEX` base
handler that resolves the array variable's element type and field. All three
bugs affected every field read in every member-access form: dot (`s.x`),
arrow (`p->x`), and subscript-then-dot (`arr[i].x`). Writing to FP struct
fields already worked correctly (the assignment path had proper guards).
No tokenizer, parser, or AST changes. All 1,863 tests pass.

**Stage 118** adds **pointer relational comparison operators** (`<`, `<=`, `>`,
`>=`) when applied to pointer operands. The grammar already accepted these
operators; the bug was that `is_pointer_cmp` in `codegen_expression()` only
handled `==` and `!=`. The fix extends `is_pointer_cmp` to cover all six
comparison operators, adds a validation guard that rejects `pointer < integer`
(only `==`/`!=` against the null pointer constant are valid per C99), and
selects unsigned `setb`/`setbe`/`seta`/`setae` setcc variants (pointer
addresses are unsigned 64-bit values). No tokenizer, parser, or AST changes.
All 1,872 tests pass.

**Stage 119** fixes six codegen bugs causing **FP compound assignment to silently
use integer instructions when both operands are fields of file-scope (global)
struct variables**. Root cause: `expr_result_type()` and `sizeof_type_of_expr()`
did not call `codegen_find_global` when `codegen_find_var` returned NULL, so FP
struct field types resolved to `TYPE_INT` for global variables. An additional
related bug in `emit_arrow_addr()` prevented `gp->field` from working as an
lvalue when `gp` is a global pointer-to-struct. All six fixes add
`codegen_find_global` fallbacks in `expr_result_type()` (AST_MEMBER_ACCESS
VAR_REF and DEREF bases, AST_ARROW_ACCESS), `sizeof_type_of_expr()`
(AST_MEMBER_ACCESS VAR_REF base, AST_ARROW_ACCESS), and `emit_arrow_addr()`
(VAR_REF base). No tokenizer, parser, or AST changes. All 1,879 tests pass.

**Stage 120** adds **FP `++`/`--` on struct members and other FP lvalues**:
`codegen_inc_dec_general()` now handles `TYPE_FLOAT` and `TYPE_DOUBLE` targets
by emitting `movsd`/`movss` loads, `addsd`/`addss` or `subsd`/`subss` with
new `.rodata` constants (`Lfp_one_f64`, `Lfp_one_f32`), and matching stores.
Covers dot (`.`), arrow (`->`), array subscript, and pointer dereference
targets. No tokenizer, parser, or AST changes. All 1,886 tests pass.

**Stage 121** fixes the **`switch` dispatch loop for 64-bit discriminants**:
when the discriminant is `TYPE_LONG`, `TYPE_LONG_LONG`, or
`TYPE_UNSIGNED_LONG_LONG`, the dispatch emits `mov rax, [rsp]` / `cmp rax, <val>`
(64-bit compare); `int`/`char`/`short` continue to use the 32-bit
`mov eax, [rsp]` / `cmp eax, <val>` path (they are integer-promoted to `int`
before evaluation). No tokenizer, parser, or AST changes. All 1,892 tests pass.

**Stage 122** fixes an **ABI violation by preserving the callee-saved `rbx`
register** across all generated functions, as required by the SysV AMD64 ABI
(§3.2.1). The compiler uses `rbx` as a scratch register for address arithmetic
(array indexing, struct member access, lvalue inc/dec) via balanced push/pop
pairs within expression subtrees. While safe intra-expression, external code
(e.g. glibc `qsort`) that calls generated functions as callbacks relies on
`rbx` being preserved across the call boundary. The fix reserves `[rbp - 8]`
in every function frame, emits `mov [rbp - 8], rbx` in the prologue (after
`sub rsp`), and emits `mov rbx, [rbp - 8]` before each of the four epilogue
paths. `cg->stack_offset` starts at 8 (was 0), and variadic functions add an
extra 8-byte alignment pad before the 176-byte XMM register-save area so
`movaps` XMM slots remain 16-byte aligned. All 21 `test/print_asm/` expected
files were regenerated to reflect the new instructions and shifted local-variable
offsets. No tokenizer, parser, or AST changes. All 1,894 tests pass.

**Stage 123** fixes five ABI correctness bugs reported in `docs/defects/CC99_FIX_REPORT-02.md`:
(CC99-001/002) FP stack arguments beyond 8 XMM registers were stored via `mov rax`
(integer store) instead of `movss`/`movsd`, fixed by adding an `is_fp` field to
`ArgSlot` and emitting the correct instruction in Phase 1; (CC99-003) indirect FP
calls used a GP-register-only push/pop path that ignored XMM registers, fixed by
routing to the `CallLayout`-based `involves_special` path whenever any argument
is FP; (CC99-004) **address-constant global initializers** — `&global` and
`&global[N]` forms are now valid pointer-global initializers, with the parser
accepting `AST_VAR_REF` (function designators) and `AST_ADDR_OF` and codegen
emitting `dq label` / `dq label + offset`; `LocalStaticVar` gains
`is_label_init`/`init_label` fields for block-scope static pointers with
address-constant initializers; (CC99-005) `&&`/`||` short-circuit evaluation
for FP operands now calls `emit_fp_bool_to_rax` before the conditional branch
instead of treating the result as an integer. All 1,901 tests pass.

**Stage 124** adds three independent language features: **octal integer literals**
(C99 §6.4.4.1) — the lexer detects `0[0-7]+` sequences; digits 8 or 9 in an
octal literal are a compile error; values are parsed with `strtoul(base=8)` and
rewritten to a decimal string for NASM output; **`__func__` predefined identifier**
(C99 §6.4.2.2) — `codegen_function` injects a `LocalStaticVar` holding the
function name as a null-terminated byte string and a `LocalVar` named `__func__`
pointing to it, reusing the existing `LocalStaticVar`/`LocalVar` infrastructure;
**file-scope compound literals with pointer types** — the early rejection of
`AST_COMPOUND_LITERAL` at file scope is relaxed to allow pointer-typed globals,
with codegen in `codegen_emit_data` handling array (Case A) and address-of (Case B)
forms via anonymous `Lcompound_N` labels. All 21 `test/print_asm/` golden files
were regenerated because every function now injects a `.data` section for `__func__`.
A bootstrap fix was required: a multi-declarator array declaration in `src/codegen.c`
was split into four separate declarations. All 1,912 tests pass; C0→C1→C2 verified.

**Stage 125** fixes two floating-point correctness bugs: **FP globals from integer
initializers** — `double x = 5;` at file scope previously emitted `dq 5` (raw
integer bits) instead of the IEEE 754 encoding; the parser now accepts
`AST_INT_LITERAL` for `float`/`double` globals and folds negated integer literals;
codegen converts via `%.17g`/`%.9g` and appends `.0` when no decimal point or
exponent is present so NASM emits the correct bit pattern; **variadic
float→double promotion** — `float` arguments in variadic calls are now promoted
to `double` per C99 §6.5.2.2p7 by emitting `cvtss2sd xmm0, xmm0` in both the
XMM-register and stack-overflow paths of `compute_call_layout`. The checklist
items "Unary + on floating-point" and "Mixed integer/FP arithmetic" that were
already implemented in Stage 110 are retroactively marked complete. All 1,919
tests pass; C0→C1→C2 all pass 1919/1919 with no source changes.

**Stages completed: 246** (stage-01 through stage-125, including substages).
...
| Total LOC (src + include)     | 17,180         |
| Valid runtime tests           | 1,233          |
...
All 1,919 tests pass with no regressions — under the gcc-built compiler (C0)
...
- **`stdlib.h`** — `malloc`, `calloc`, `free`, `realloc`, `exit`,
  `strtol`, `strtoul`, `qsort`, `bsearch`, `abs`.
...
- **Callee-saved `rbx` preserved** in every function prologue and epilogue
  per the SysV AMD64 ABI — stage 122.
...
- `switch` / `case` / `default` (linear dispatch; nested switches OK).
  ...
  **64-bit discriminants** (`long`, `long long`, `unsigned long long`) use
  `cmp rax` — stage 121.
...
- Integer literal forms: decimal, **`0x`/`0X` hexadecimal** literals (stage 90),
  and **`0NNN` octal** literals (stage 124) with `U`/`L`/`UL`/`LL`/`ULL`
  suffixes and overflow-aware typing. Octal digits 8 and 9 are rejected.
...
- **Compound literals** (`(type-name){ initializer-list }`) — unnamed
  ...
  pointer-type compound literals** now also supported (stage 124).
- File-scope (global) variable declarations (`.bss` / `.data`,
  RIP-relative addressing). **Address-constant global pointer initializers**
  (`&global`, `&global[N]`, function designators) supported since stage 123.
- **FP global initializers from integer literals**: `double x = 5;` and
  `float x = -7;` at file scope correctly emit IEEE 754 encoding — stage 125.
...
- **`__func__` predefined identifier** (C99 §6.4.2.2, stage 124): available
  in any function body as a `const char *` pointer to the function's name.
...
- **File-scope pointer-type compound literals** (stage 124): pointer globals
  may be initialized from array or address-of compound literals.
...
- Uninitialized global and block-scope static struct/union arrays correctly
  reserve `resb (N × sizeof(struct))` bytes in BSS. Global `char[N]`
  sub-arrays and struct `char[N]` fields initialized from string literals are
  emitted as inline bytes — stage 116.

### Floating-point types and calling convention (stages 109–125)
...
- **FP `++`/`--` on any FP lvalue** (stage 120): `codegen_inc_dec_general()`
  handles `TYPE_FLOAT`/`TYPE_DOUBLE` targets via `movss`/`movsd` loads and
  `addss`/`addsd` or `subss`/`subsd` with `.rodata` constants (`Lfp_one_f64`,
  `Lfp_one_f32`). Covers dot (`.`), arrow (`->`), array subscript, and deref.
...
- **FP logical short-circuit** (stage 123): `&&`/`||` operators call
  `emit_fp_bool_to_rax` when an operand is FP, preserving C short-circuit
  semantics for float/double boolean expressions.
...
- **Variadic float→double promotion** (stage 125): `float` arguments in
  variadic calls are promoted to `double` per C99 §6.5.2.2p7; `cvtss2sd
  xmm0, xmm0` is emitted for float variadic extras in both the XMM-register
  and stack-overflow paths.
- **FP struct member rvalue reads** (stage 117): `double`/`float` fields
  read via dot (`s.x`), arrow (`p->x`), and subscript-then-dot (`arr[i].x`)
  now correctly load into `xmm0` using `movsd`/`movss` instead of the
  erroneous 4-byte integer `mov eax, [rax]`; `expr_result_type()` now
  correctly reports `TYPE_DOUBLE`/`TYPE_FLOAT` for FP struct fields so
  binary operators on those fields take the SSE2 code path.
- **FP compound assignment on global struct fields** (stage 119):
  `expr_result_type()` and `sizeof_type_of_expr()` now correctly resolve FP
  field types for file-scope struct variables (not just locals) via
  `codegen_find_global` fallbacks in AST_MEMBER_ACCESS and AST_ARROW_ACCESS
  cases. `emit_arrow_addr()` gains a global pointer-to-struct fallback so
  `gp->field` works as an lvalue when `gp` is a file-scope variable.
  Compound assignment operators (`+=`, `-=`, `*=`, `/=`) on global struct FP
  fields now correctly emit SSE2 arithmetic rather than integer instructions.
- **FP globals from integer initializers** (stage 125): `double x = 5;` and
  `float x = -7;` at file scope emit the correct IEEE 754 bit pattern (`dq 5.0`,
  not `dq 5`). The parser accepts `AST_INT_LITERAL` for FP globals and folds
  negated integer literals; codegen formats via `%.17g`/`%.9g` and appends
  `.0` if no decimal point or exponent is present.
- **Indirect FP calls** (stage 123): `AST_INDIRECT_CALL` for function pointers
  now uses the `CallLayout`-based `involves_special` path for any FP argument
  (was the GP-register-only push/pop path, which silently discarded XMM args).
- **FP stack arguments** (stage 123): FP arguments beyond the 8 XMM registers
  now use `movss`/`movsd` (not `mov rax`) in the Phase 1 stack-passing path.

### Pointer comparisons (stages 31–118)
- **Equality and inequality** (`==`, `!=`) on pointers (stage 31); null
  pointer constant check; pointer equality with null constant `0`.
- **Relational comparison** (`<`, `<=`, `>`, `>=`) on pointer operands
  (stage 118): unsigned `setb`/`setbe`/`seta`/`setae` variants; rejects
  `pointer < integer` (only `==`/`!=` against a null constant are valid
  per C99).

### Variadic functions (`<stdarg.h>`)
- `va_start` / `va_end` / `va_arg` for GP-class types (int, long, pointers)
  and **`double`** (stage 112).
- **`float` variadic arguments promoted to `double`** per C99 §6.5.2.2p7
  (stage 125): `cvtss2sd` emitted in both XMM-register and stack-overflow
  paths.
- `va_copy` is supported via three 8-byte moves copying the 24-byte
  `va_list` struct (stage 107).
- `va_arg(ap, float)` is rejected per C99 §6.5.2.2p6.

### Expressions
- Integer (decimal/hex/octal), string, and character literals; adjacent
  string-literal concatenation; hex/octal escapes; variable references.
- **Octal integer literals** `0NNN` (stage 124): lexer parses via
  `strtoul(base=8)` and rewrites to decimal for NASM; digits 8/9 rejected.
- **Float and double literals** with `f`/`F` suffix detection.
- **`__func__` predefined identifier** (stage 124): `const char *` pointer
  to the enclosing function name, available in any function body.
...
- **FP struct member reads** via dot, arrow, and subscript-then-dot (stage 117).
- **FP compound assignment on global struct fields** (stage 119).
- **FP `++`/`--` on struct members and other FP lvalues** (stage 120).
- **Pointer relational comparisons** (`<`, `<=`, `>`, `>=`) — stage 118.
...
- **FP struct member rvalue loads**: `movss xmm0, [rax]` / `movsd xmm0, [rax]`
  for `TYPE_FLOAT`/`TYPE_DOUBLE` fields in `AST_MEMBER_ACCESS` and `AST_ARROW_ACCESS`
  rvalue blocks (stage 117).
- **FP compound assignment on global structs**: `codegen_find_global` fallbacks
  in `expr_result_type()`, `sizeof_type_of_expr()`, and `emit_arrow_addr()`
  (stage 119).
- **FP `++`/`--`**: `codegen_inc_dec_general()` handles FP lvalues using
  `movss`/`movsd`, `addss`/`addsd`/`subss`/`subsd`, and `.rodata` constants
  `Lfp_one_f64`/`Lfp_one_f32` (stage 120).
- **SysV FP calling convention**: `compute_call_layout` classifies args as
  ...
  params to local slots; float variadic args promoted to double via `cvtss2sd`
  (stage 125).
- **Indirect FP calls**: `AST_INDIRECT_CALL` uses `CallLayout`-based spill/
  restore path whenever any argument is FP (stage 123).
- **FP stack args**: Phase 1 uses `movss`/`movsd` for FP arguments beyond 8
  XMM registers (stage 123).
- **FP logical short-circuit**: `&&`/`||` emit `emit_fp_bool_to_rax` for FP
  operands before the conditional branch (stage 123).
- **FP globals from integer initializers**: `codegen_add_global` converts integer
  strings to IEEE 754 decimal strings for NASM `dq`/`dd` directives (stage 125).
- **`__func__` injection**: `codegen_function` injects `LocalStaticVar`/`LocalVar`
  at every function entry for the `__func__` predefined string (stage 124).
- **Address-constant globals**: `codegen_emit_data` emits `dq label` /
  `dq label + offset` for `&global` and `&global[N]` initializers (stage 123).
- **File-scope pointer compound literals**: `codegen_emit_data` handles
  anonymous `Lcompound_N` labels for array and address-of forms (stage 124).
- **Pointer relational comparisons**: unsigned setcc variants (`setb`/`setbe`/
  `seta`/`setae`) for `<`/`<=`/`>`/`>=` on pointer operands (stage 118).
- **64-bit switch discriminants**: `mov rax, [rsp]` / `cmp rax, val` for
  `long`/`long long`/`unsigned long long` discriminants (stage 121).
- **Callee-saved `rbx` preservation**: every function prologue emits
  `mov [rbp - 8], rbx` after `sub rsp`; every epilogue emits
  `mov rbx, [rbp - 8]` before `mov rsp, rbp` (stage 122).
...
## Stage-by-Stage Timeline (76–125)
...
| 116        | Global struct array BSS fix; char[N] string-literal init      |
| 117        | FP struct member rvalue reads (dot, arrow, subscript-then-dot)|
| 118        | Pointer relational operators (`<`, `<=`, `>`, `>=`) on pointers|
| 119        | FP compound assignment on global struct fields                |
| 120        | FP `++`/`--` on struct members and other FP lvalues           |
| 121        | `switch` on 64-bit discriminants (`long`, `long long`)        |
| 122        | Callee-saved `rbx` preserved per SysV AMD64 ABI               |
| 123        | ABI bug fixes: FP stack args, indirect FP calls, address-constant globals, FP short-circuit |
| 124        | Octal literals, `__func__`, file-scope pointer compound literals |
| **125**    | **FP globals from integer initializers; variadic float→double promotion (current)** |

## Recently Shipped (Stages 123–125)

**Stage 123 — ABI Bug Fixes.** Fixes five correctness bugs reported in
`docs/defects/CC99_FIX_REPORT-02.md`. (CC99-001/002) FP stack arguments beyond
8 XMM registers were stored using `mov rax` (integer) instead of `movss`/`movsd`;
fixed by adding an `is_fp` field to `ArgSlot` and switching the Phase 1 emission
to the appropriate FP instruction. (CC99-003) `AST_INDIRECT_CALL` used a
GP-register-only push/pop path that ignored XMM arguments; fixed by routing to
the `CallLayout`-based `involves_special` path for any FP argument. (CC99-004)
`&global` and `&global[N]` are now valid file-scope pointer initializers: parser
accepts `AST_VAR_REF` and `AST_ADDR_OF`; codegen emits `dq label` / `dq label +
offset`; `LocalStaticVar` gains `is_label_init`/`init_label` fields for
block-scope static pointer initializers. (CC99-005) `&&`/`||` operators now call
`emit_fp_bool_to_rax` for FP operands before the conditional branch, preserving
C short-circuit evaluation for float/double. 7 new valid tests. All 1,901 tests
pass; self-host C0→C1→C2 verified with no source changes.

**Stage 124 — Octal Literals, `__func__`, File-Scope Compound Literals.**
Adds three independent language features. **Octal integer literals** (`0NNN`):
the lexer detects `0[0-7]+`; digits 8 or 9 trigger a compile error; values are
parsed via `strtoul(base=8)` and rewritten to decimal for NASM output (C99 octal
syntax is not valid NASM). **`__func__` predefined identifier** (C99 §6.4.2.2):
`codegen_function` injects a `LocalStaticVar` holding the null-terminated function
name and a `LocalVar` named `__func__` pointing to it at function entry; no parser
changes are needed (stage 106/107 already handled `__func__` as a VAR_REF in
`parse_primary`). **File-scope pointer-type compound literals**: the blanket
rejection of `AST_COMPOUND_LITERAL` at file scope is replaced with a type-specific
guard; pointer globals may now use array or address-of compound literals as
initializers; codegen in `codegen_emit_data` handles both cases via anonymous
`Lcompound_N` labels. All 21 `test/print_asm/` golden files regenerated (every
function now has a `.data` section for `__func__`). A bootstrap fix split one
multi-declarator array declaration in `src/codegen.c` into four separate
declarations. 12 new tests across arithmetic, functions, and pointers categories.
All 1,912 tests pass; C0→C1→C2 verified (1912/1912 at each step).

**Stage 125 — FP Globals from Integer Initializers; Variadic Float Promotion.**
Fixes two FP correctness bugs. **FP globals from integer initializers**: `double x = 5;`
at file scope previously emitted `dq 5` (raw integer bits 0x0000000000000005),
not the IEEE 754 double encoding of 5.0. The parser now accepts `AST_INT_LITERAL`
for `TYPE_FLOAT`/`TYPE_DOUBLE` globals and folds negated integer literals
(`-7` stored as a new `AST_INT_LITERAL`). Codegen formats the integer as a
decimal string with `%.17g` (double) or `%.9g` (float), appending `.0` if no
decimal point or exponent is present, then stores it via `init_label` so NASM
treats it as a floating-point constant. **Variadic float→double promotion**:
`float` arguments in variadic calls were not promoted to `double` as required
by C99 §6.5.2.2p7. `compute_call_layout` now assigns `nbytes = 8` for float
variadic extras, and both the XMM-register (Phase 2) and stack-overflow (Phase 1)
paths emit `cvtss2sd xmm0, xmm0` before storing the argument. 7 new valid tests.
All 1,919 tests pass; C0→C1→C2 all pass 1919/1919 with no source changes.

## Out of Scope (Not Yet Implemented)

- `long double` (x87 80-bit; explicitly out of scope).
- `va_arg` for struct-by-value types; `va_arg(ap, float)` (rejected per C99);
  `va_copy` codegen (now implemented via three 8-byte moves since stage 107).
- Multi-character constants (`'ab'`); wide-character literals.
- Non-pointer compound literals at file scope (pointer-type compound literals
  at file scope now work — stage 124).
...
├── lexer.c              Tokenizer (line/col positions; hex/octal literals; hex/octal escapes; FP literals)
...
├── codegen.c            Single-pass walker → NASM Intel-syntax asm; SysV ABI; SSE2 FP; rbx ABI; __func__
...
in `docs/other/stage-125-parse-tree.md`, and the feature checklist in
