# ClaudeC99 Project Status — Through Stage 119

_As of 2026-06-14 (commit `40285e7`)_
...
**Stages completed: 240** (stage-01 through stage-119, including substages).
...
| Total LOC (src + include)     | 16,716         |
| Valid runtime tests           | 1,195          |
...
All 1,879 tests pass with no regressions — under the gcc-built compiler (C0)
...
### Floating-point types and calling convention (stages 109–119)
...
## Stage-by-Stage Timeline (76–119)
...
| **119**    | **FP compound assignment on global struct fields (current)**  |

## Recently Shipped (Stages 117–119)

**Stage 117 — FP Struct Member Rvalue Reads.** Three codegen bugs fixed that
caused silent data corruption when reading `double`/`float` struct fields.
(1) Rvalue load bug: `codegen_expression()` rvalue blocks for `AST_MEMBER_ACCESS`
and `AST_ARROW_ACCESS` emitted `mov eax, [rax]` (4-byte integer load) for FP
fields instead of `movsd xmm0, [rax]` / `movss xmm0, [rax]`. Fixed by adding
FP early-return blocks (identical in structure to the existing array member
decay blocks). (2) Type inference bug: `expr_result_type()` called
`promote_kind(TYPE_DOUBLE)` = `TYPE_INT` for FP struct fields, causing binary
operators (`a.x - b.x`) to take the integer arithmetic path. Fixed by adding
`else if (type_is_fp(f->kind)) { t = f->kind; }` in all three affected base
cases. (3) Subscript-then-member fallthrough: neither `expr_result_type()` nor
`sizeof_type_of_expr()` had a handler for `AST_ARRAY_INDEX` bases (`arr[i].x`),
falling through to `TYPE_INT`. Fixed by adding an AST_ARRAY_INDEX base handler
that resolves the array variable, its element type, and the named struct field.
All three bugs affected dot (`s.x`), arrow (`p->x`), and subscript-then-dot
(`arr[i].x`) access forms. The write (lvalue assignment) path was already
correct. Six new valid tests. All 1,863 tests pass.

**Stage 118 — Pointer Relational Operators.** Extends `is_pointer_cmp` in
`codegen_expression()` AST_BINARY_OP to cover all six comparison operators
(was `==`/`!=` only). Adds validation guard `is_relcmp` that rejects
`pointer < integer` before the null-pointer-constant check (only `==`/`!=`
against null are valid per C99). Selects unsigned `setb`/`setbe`/`seta`/`setae`
setcc variants for pointer relational comparisons (pointer addresses are unsigned
64-bit values). No tokenizer, parser, or AST changes. 9 new tests (7 valid
pointer comparison patterns, 2 invalid rejections). All 1,872 tests pass.

**Stage 119 — FP Compound Assignment on Global Struct Fields.** Six codegen bugs
fixed. Root cause: `expr_result_type()` and `sizeof_type_of_expr()` did not call
`codegen_find_global` when `codegen_find_var` returned NULL, so both functions
fell back to `TYPE_INT` for FP fields of file-scope struct variables — causing
compound assignment operators to silently use integer arithmetic instructions on
IEEE 754 bit patterns. An additional related bug in `emit_arrow_addr()` prevented
`gp->field` from working as an lvalue when `gp` is a global pointer-to-struct.
All six fixes add `codegen_find_global` fallbacks: in `expr_result_type()`
(AST_MEMBER_ACCESS VAR_REF base, AST_MEMBER_ACCESS DEREF base, AST_ARROW_ACCESS),
in `sizeof_type_of_expr()` (AST_MEMBER_ACCESS VAR_REF base, AST_ARROW_ACCESS),
and in `emit_arrow_addr()` (VAR_REF base). No tokenizer, parser, or AST changes.
7 new valid tests. All 1,879 tests pass.
...
in `docs/other/stage-119-parse-tree.md`, and the feature checklist in
