# ClaudeC99 Project Status — Through Stage 95-12

_As of 2026-06-08 (commit `3c41047`)_
...
writes anywhere and no hard cap on case/default labels per switch. No language
features changed in this range.

**Stages completed: 202** (stage-01 through stage-95-12, including substages).
...
| Total LOC (src + include)     | 13,728         |
| Unit-test assertions          | 165            |
| Valid runtime tests           | 833            |
...
All 1,481 tests pass with no regressions — under the gcc-built compiler and
...
## Stage-by-Stage Timeline (76–95-12)
...
| **95-12**  | **`#if` unary buffer → `StrBuf`; switch labels → `Vec` (current)** |

## Recently Shipped (Stages 94–95-12)

This range adds **no language features** — it hardens the bootstrap workflow
and replaces every remaining fixed-capacity table and fixed-size text buffer
in the compiler with dynamic storage. All work was re-verified self-hosting
(C0→C1→C2) at each stage.

**Stage 94 — repeatable self-host cycle.** `build.sh` gains
`--mode=self-host`, which runs the full C0→C1→C2 cycle end-to-end: it archives
prior binaries to `build/previous/`, does a fresh cmake build to produce C0
(always gcc-built), bootstraps C0→C1 and C1→C2, runs the entire suite at each
step, and commits a checkpoint between stages. Five build-infrastructure
defects were fixed (missing `-I test/include` for stub headers during
bootstrap, missing `-DVERSION_BUILD` computation/passing, missing C0/C1 and
C1/C2 commit checkpoints, and a `VERSION_BUILTBY` regex that captured only
three of four version groups). No compiler code changed.

**Stages 95-01 → 95-03 — inventory and dynamic containers.** Stage 95-01 is a
documentation-only audit (`docs/fixed-capacity-inventory.md`) cataloging all 23
fixed-capacity tables and buffers with overflow behavior, aliasing, and a
priority ranking. Stage 95-02 adds the reusable `Vec` generic growable-array
module (`vec_init`/`free`/`reserve`/`push`/`get`/`pop`/`clear`; lazy doubling
growth, overflow checks, fatal on OOM) and a new `test/unit/` suite (106
assertions). Stage 95-03 adds the complementary `StrBuf` dynamic character
buffer (59 more assertions, 165 unit total).

**Stages 95-04 → 95-07 — tables become dynamic.** Fourteen fixed-capacity
parser/codegen arrays were converted to `Vec`: `enum_tags`, `union_tags`,
`globals`, `enum_consts`, `struct_tags`, `typedefs`, `funcs`, `locals`,
`globals`, `string_pool`, `break_stack`, `switch_stack`, `local_statics`,
`user_labels`, and the struct/union field-parsing scratch buffer — removing
their hard caps. Three latent overflow bugs were fixed in passing: struct
fields beyond 64 were silently dropped (hardcoded `64` check), `break_stack`
had no bounds check, and `compute_call_layout` had no guard against >24
arguments. Several constants were retired from `include/constants.h`.

**Stages 95-08 → 95-11 — text becomes pointer-based.** All identifier, tag,
and label storage migrated from `char[MAX_NAME_LEN]` embedded buffers to
`const char *` pointers backed by a lexer-owned string pool. Stage 95-08
redefined `Token` with `const char *lexeme`/`value` plus `size_t` lengths and
added `lexer_store_bytes()`; this removed the 255-byte string-literal limit and
enabled embedded NUL bytes (string literals now decode through a `StrBuf`).
Stage 95-09 changed `ASTNode.value` and `ParsedDeclarator.name` to pointers
(and fixed a use-after-stack bug in case-label formatting that had been failing
all switch tests). Stages 95-10 and 95-11 converted the seven parser.h and five
codegen.h name/tag/label fields, replaced the 2-D `user_labels` array with a
`Vec` of pointers, and removed `MAX_USER_LABELS`. (`MAX_NAME_LEN` lingered on
`StructField.name` until that field too became a pointer; the now-dead constant
was removed in commit `9fda93c`.)

**Stage 95-12 — the last two fixed-capacity items.** The preprocessor's
`eval_cond_unary` replaced its unchecked `char ops[32]` (a `#if` with >32
chained unary operators overran the stack and crashed) with a dynamic `StrBuf`;
operators are appended as consumed and applied right-to-left. `SwitchCtx`
replaced its parallel fixed arrays `nodes[256]`/`labels[256]` with an embedded
`Vec` of a new `SwitchLabel{node,label}` pair struct, removing the
`MAX_SWITCH_LABELS` per-switch cap (constant deleted). The `Vec` is init'd
before the `SwitchCtx` is pushed onto `switch_stack` and freed before it is
popped, re-fetching the live top element after the body so nested switches that
reallocate the stack cannot dangle it (a bug caught by the existing
nested-20-deep switch test). With these two conversions there are no unchecked
fixed-capacity writes anywhere and no hard cap on case/default labels; the only
remaining fixed limits are an intentionally-static set (`FUNC_MAX_PARAMS`,
`FUNC_TYPE_MAX_PARAMS`, `MAX_ARRAY_DIMS`, `MAX_INCLUDE_DEPTH`, `MAX_COND_DEPTH`,
`MAX_CALL_LAYOUT_ITEMS`) that overflow cleanly with a diagnostic. Two tests
added (100-operator `#if` chain; 300-case switch).
...
in `docs/other/stage-95-12-parse-tree.md`, and the feature checklist in
