# ClaudeC99 Project Status — Through Stage 130

_As of 2026-06-15 (commit `c2bd2ad`)_
...
**Stages completed: 237** (stage-01 through stage-130, including substages).
...
| Total LOC (src + include)     | 17,494         |
| Valid runtime tests           | 1,251          |
| Invalid (compile-error) tests | 259            |
| Print-asm golden tests        | 21             |
| **Total tests**               | **1,531**      |
| Git commits                   | 978            |

All 1,531 tests pass with no regressions — under the gcc-built compiler and
...
- **`stdlib.h`** — `malloc`, `calloc`, `free`, `realloc`, `exit`,
  `strtol`, `strtoul`, plus `div_t`/`ldiv_t`/`lldiv_t`, `EXIT_SUCCESS`/
  `EXIT_FAILURE`/`RAND_MAX`, and the full stage-106 function set.
...
## Stage-by-Stage Timeline (100–130)
...
## Recently Shipped (Stages 126–130)

**Stage 126 — Tentative definitions (C99 §6.9.2).**

Multiple file-scope declarations of the same variable without initializers
are now merged as tentative definitions. `GlobalObjSig` gains an `is_defined`
field; `parser_register_global()` accepts re-registration when the existing
entry has no initializer. Codegen fast-path handles duplicate tentatives
silently, upgrades `extern T x; T x;` by clearing `is_extern`, and updates
the existing `GlobalVar` entry in-place when a completing initializer arrives.
Six new valid tests; one test moved from invalid to valid.

**Stage 127 — Callee-saved registers r12–r15.**

Extends stage-122 callee-saved register work to r12–r15, completing full SysV
AMD64 ABI compliance for all five callee-saved GP registers. `cg->stack_offset`
starts at 40 (was 8) to reserve five 8-byte save slots. Prologue emits
`mov [rbp - 16..40], r12..r15` after the rbx save; all four epilogue paths
restore all four registers before `mov rsp, rbp`. All 21 print-asm golden files
regenerated (local variable offsets shifted by 32 bytes). One new valid test.

**Stage 128 — FP constant expressions at file scope.**

Adds a recursive-descent floating-point constant expression evaluator
(`eval_fp_const_expr`, `eval_fp_mult`, `eval_fp_unary`, `eval_fp_primary`)
that operates on the token stream and accumulates a `double` result. The
`TYPE_FLOAT` / `TYPE_DOUBLE` global initializer path now calls this evaluator
instead of `parse_assignment_expression`, enabling `double x = 3.14159 * 2.0;`
and `float eps = 1.0f / 1024.0f;` at file scope. Four new valid tests.

**Stage 129 — Block-scope function declarations and extern incomplete arrays.**

Two orthogonal syntactic restrictions are lifted. Block-scope function
declarations (`void foo(void);` inside a compound block) are now parsed:
the parameter list is skipped with a depth counter, an `AST_TYPEDEF_DECL`
no-op is emitted, and the function is registered with `param_count = -1`
(unknown arity) so any call-site arity is accepted without error. Extern
incomplete arrays (`extern int arr[];`) are now accepted by the parser (was
rejected); the codegen update path sets the completing definition's length on
the `GlobalVar` so later subscript uses get the correct type. Three new valid
tests.

**Stage 130 — `va_arg` for struct/union by value (SysV AMD64 ABI).**

Implements the most complex remaining variadic codegen task. The SysV AMD64
ABI classifies struct/union arguments in three cases:

- **MEMORY class** (size > 16 bytes): pointer loaded from `overflow_arg_area`,
  `size` bytes copied to a scratch slot via `rep movsb`, overflow pointer
  advanced by `size` rounded to 8.
- **Register class 1 eightbyte** (size ≤ 8): checks `gp_offset < 48`;
  loads 8 bytes from `reg_save_area + gp_offset` or `overflow_arg_area`;
  copies to scratch; advances the appropriate pointer.
- **Register class 2 eightbytes** (size 9–16): checks `gp_offset ≤ 32`;
  loads two consecutive 8-byte slots; advances by 2×8 or from overflow.

Supporting changes: `compute_struct_ret_bytes` pre-allocates scratch slots for
`AST_BUILTIN_VA_ARG` struct nodes; `emit_struct_addr` gains an
`AST_BUILTIN_VA_ARG` case; `compute_call_layout` gains a `CodeGen *cg`
parameter and looks up struct `full_type` from variable tables; `expr_result_type`
now returns `TYPE_STRUCT` / `TYPE_UNION` for struct/union variables;
`involves_special` updated to detect struct args. One test moved from invalid to
valid; three new valid tests.
...
├── version.c            Build/version identifier (VERSION_STAGE = "01300000")
...
in `docs/other/stage-130-parse-tree.md`, and the feature checklist in
