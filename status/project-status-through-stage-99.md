# ClaudeC99 Project Status — Through Stage 99

_As of 2026-06-10 (commit `6c2b9e6`)_
...
**Stages completed: 206** (stage-01 through stage-99, including substages).
...
| Total LOC (src + include)     | 14,573         |
| Valid runtime tests           | 864            |
| Invalid (compile-error) tests | 246            |
| Integration tests             | 86             |
| Print-AST golden tests        | 49             |
...
| **99**     | **`typedef enum` completion — const expr evaluator + forward refs (current)** |

## Recently Shipped (Stage 99)

**Stage 99 — typedef enum completion.**

Two remaining gaps in the `typedef enum` feature are closed.

**Gap 1 — Enumerator values accept full constant expressions.**

The three-function `eval_case_const_*` chain (additive only, literals
and enum constants only) is replaced by a nine-level recursive descent
evaluator `eval_const_expr`:
...
A `const char *context` parameter is threaded through the chain so error
messages are specific: "case label expression is not …", "enumerator value
is not …", "array designator index is not …". Division/modulo by zero
produces a `PARSER_ERROR` ("division by zero in constant expression").

The enumerator assignment arm in `parse_enum_specifier` is reduced to a
single call: `next_val = eval_const_expr(parser, "enumerator value");`,
replacing the old literal-only check with trailing-comma guard.

**Gap 2 — Forward-declared enum tags.**

The no-body `enum Tag` path in `parse_enum_specifier` previously errored
when `Tag` was not in the `enum_tags` table. The fix creates a
forward-declaration entry (`EnumTag { tag, is_defined=0 }`) and returns
`type_int()` immediately. When the body appears for the same tag, the
existing search-and-mark logic (`is_defined=1`) completes the reference.
This enables:

```c
typedef enum Status Status;      /* forward ref — creates placeholder */
enum Status { OK = 0, ERR = 1 }; /* body — marks placeholder defined  */
Status check(int v) { return v ? ERR : OK; }
```

The same evaluator improvement makes `case 1 << 2:` (evaluates to 4) and
array designator `[2 + 1] = value` work through the shared `eval_const_expr`.

Nine valid tests, two invalid tests, and one print-AST golden test were added;
two outdated invalid tests (expressions in enum values, undefined tag) were
removed. Net: +10 valid, ±0 invalid, +1 print-AST. All 1531 tests pass.
...
in `docs/other/stage-99-parse-tree.md`, and the feature checklist in
