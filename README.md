...
variadics; expression evaluation in `#if`/`#elif` (integer constants, `defined(NAME)`, `defined NAME`, object-like macro identifiers expanding to integers, unary operators, equality/relational comparison operators (`==`, `!=`, `<`, `<=`, `>`, `>=`), and logical operators (`&&`, `||`) are supported, but binary arithmetic, bitwise, and shift operators are not), `#elifdef`/`#elifndef`; pointer-to-function-pointer and function-returning-function-pointer;
...
`Aggregate: P passed, F failed, T total` line. As of stage 55-07 all
tests pass (610 valid, 192 invalid, 27 integration, 39 print-AST,
99 print-tokens, 21 print-asm; 988 total).
