...
variadics; expression evaluation in `#if`/`#elif` (integer constants, `defined(NAME)`, `defined NAME`, object-like macro identifiers expanding to integers, unary operators, equality/relational comparison operators (`==`, `!=`, `<`, `<=`, `>`, `>=`), logical operators (`&&`, `||`), and arithmetic operators (`+`, `-`, `*`, `/`, `%`) are supported, but bitwise and shift operators are not), `#elifdef`/`#elifndef`; pointer-to-function-pointer and function-returning-function-pointer;
...
`Aggregate: P passed, F failed, T total` line. As of stage 55-08 all
tests pass (614 valid, 194 invalid, 27 integration, 39 print-AST,
99 print-tokens, 21 print-asm; 994 total).
